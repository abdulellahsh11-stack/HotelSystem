  ...
  <div class="content">
    ...
    <div class="pg" id="pg-backup">
      ...
      <div class="card cg">
        ...
        <div style="font-size:11px;color:var(--muted);margin-bottom:10px">تُحفظ جميع البيانات (نزلاء، موردين، فواتير، قيود...) في ملف Excel منظّم — يفتح مباشرة في Excel أو Sheets</div>
        <div class="g2">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          <button class="btn bp" onclick="downloadBackupNow()" data-tt="تنزيل ملف Excel منظّم — لا يُرسل لأي خادم">⬇️ تنزيل على جهازي</button>
...
function downloadBackupNow(){
  ...
  // ── النزلاء ──
  rows.push("النزلاء");
  rows.push("رقم الحجز,اسم النزيل,رقم الهوية,الوحدة,نوع السرير,تاريخ الدخول,تاريخ الخروج,عدد الليالي,السعر,الإجمالي,طريقة الدفع,الحالة");
  (S.guests||[]).forEach(function(g){
    rows.push([g.id,g.name,g.idNum,g.unit,g.bed,g.inDate,g.outDate,g.nights,g.price,g.total,g.pay,g.status].map(function(v){return '"'+(v||'').toString().replace(/"/g,'""')+'"';}).join(','));
  });
  rows.push("---");
  // ── الفواتير ──
  rows.push("الفواتير");
  rows.push("رقم الفاتورة,اسم العميل,التاريخ,الإجمالي,الحالة,الملاحظات");
  (S.invoices||[]).forEach(function(inv){
    rows.push([inv.id,inv.client,inv.date,inv.total,inv.status,inv.notes].map(function(v){return '"'+(v||'').toString().replace(/"/g,'""')+'"';}).join(','));
  });
  rows.push("---");
  // ── الموردون ──
  rows.push("الموردون");
  rows.push("اسم المورد,الفئة,الجوال,الإيميل,الرصيد");
  (S.suppliers||[]).forEach(function(s){
    rows.push([s.name,s.category,s.phone,s.email,s.balance].map(function(v){return '"'+(v||'').toString().replace(/"/g,'""')+'"';}).join(','));
  });
  rows.push("---");
  // ── القيود المحاسبية ──
  rows.push("القيود المحاسبية");
  rows.push("التاريخ,البيان,مدين,دائن,الحساب");
  (S.journal||[]).forEach(function(j){
    rows.push([j.date,j.desc,j.debit,j.credit,j.account].map(function(v){return '"'+(v||'').toString().replace(/"/g,'""')+'"';}).join(','));
  });
  rows.push("---");
  // ── المصروفات ──
  rows.push("المصروفات");
  rows.push("التاريخ,البيان,المبلغ,الفئة");
  (S.expenses||[]).forEach(function(e){
    rows.push([e.date,e.desc,e.amount,e.category].map(function(v){return '"'+(v||'').toString().replace(/"/g,'""')+'"';}).join(','));
  });
  rows.push("---");
  // ── الإعدادات ──
  rows.push("الإعدادات");
  rows.push("اسم المنشأة,النوع,المدينة,عدد الغرف,تاريخ النسخة");
  var st=S.settings||{};
  rows.push([st.name,st.type,st.city,st.rooms,dateStr].map(function(v){return '"'+(v||'').toString().replace(/"/g,'""')+'"';}).join(','));
  // ── تنزيل ──
  var csv=rows.join('\n');
  ...
  a.href=url;
  a.download='backup_'+dateStr+'.csv';
  document.body.appendChild(a);a.click();document.body.removeChild(a);
  URL.revokeObjectURL(url);
  toast_('تم تنزيل النسخة الاحتياطية بصيغة Excel');
}

async function createBackup(){const r=await api('/api/backup/create',{label:gv('bk-label')});if(r.ok){ss('bk-last',r.date);await rBackup();toast_('تم إنشاء النسخة الاحتياطية ✓');}else toast_('فشل إنشاء النسخة',false);}
async function restoreBackup(name){if(!confirm(`هل أنت متأكد من استعادة: ${name}؟\nستُفقد البيانات الحالية (سيتم حفظ نسخة تلقائية أولاً)`))return;const r=await api('/api/backup/restore',{name});if(r.ok){await reload();rDash();toast_('تمت الاستعادة ✓ — البيانات محدّثة');}else toast_(r.error||'فشل الاستعادة',false);}
function downloadBackupNow(){
  // ── بناء ملف Excel بأوراق منفصلة ──
  const dt   = new Date();
  const date = dt.toISOString().slice(0,10);
  const time = dt.toLocaleTimeString('ar-SA');
  const BOM  = '﻿';

  // دالة تحويل مصفوفة كائنات إلى CSV
  function toCSV(rows, headers){
    if(!rows||!rows.length) return BOM + headers.join(',') + '
(لا توجد بيانات)
';
...
');
  ...
  const guests   = (S.guests||[]).map(g=>({
    ...
    'الوحدة': g.unit||'',
    'نوع السرير': g.bed||'',
    'تاريخ الدخول': g.inDate||'',
    'تاريخ الخروج': g.outDate||'',
    'عدد الليالي': g.nights||'',
    'السعر': g.price||'',
    'الإضافات': g.extra||'',
    'الإجمالي': g.total||'',
    'طريقة الدفع': g.pay||'',
  ...
  // ── ورقة 6: الإعدادات ──
  const settings = [{
    'اسم المنشأة': (S.settings||{}).name||'',
    'النوع': (S.settings||{}).type||'',
    'المدينة': (S.settings||{}).city||'',
    'عدد الغرف': (S.settings||{}).rooms||'',
    'تاريخ النسخة': date,
    'وقت النسخة': time
  }];

  // ── تنزيل CSV لكل ورقة مضموم في ملف واحد مع فواصل ──
  const sheets = [
    { name:'النزلاء',      headers:Object.keys(guests[0]||{'رقم الحجز':'','اسم النزيل':'','رقم الهوية':'','الوحدة':'','نوع السرير':'','تاريخ الدخول':'','تاريخ الخروج':'','عدد الليالي':'','السعر':'','الإضافات':'','الإجمالي':'','طريقة الدفع':'','الحالة':'','الوقت':''}), data:guests },
    { name:'الفواتير',     headers:['رقم الفاتورة','اسم العميل','التاريخ','الإجمالي','الحالة','الملاحظات'], data:invoices },
    { name:'الموردون',     headers:['اسم المورد','الفئة','الجوال','الإيميل','الرصيد'], data:suppliers },
    { name:'القيود',       headers:['التاريخ','البيان','مدين','دائن','الحساب'], data:journal },
    { name:'المصروفات',    headers:['التاريخ','البيان','المبلغ','الفئة'], data:expenses },
    { name:'الإعدادات',   headers:['اسم المنشأة','النوع','المدينة','عدد الغرف','تاريخ النسخة','وقت النسخة'], data:settings },
  ];

  // دمج كل الأوراق في CSV واحد مع فاصل واضح
  let combined = BOM;
  sheets.forEach(sh=>{
    combined += `\n${'='.repeat(60)}\n`;
    combined += `📋 ${sh.name} (${sh.data.length} سجل)\n`;
    combined += `${'='.repeat(60)}\n`;
    const rows = [sh.headers.join(',')];
    sh.data.forEach(r => {
    ...
    if(!sh.data.length) rows.push('(لا توجد بيانات)');
    combined += rows.join('\n') + '\n';
  });
  ...
  a.href     = url;
  a.download = `نسخة_احتياطية_${date}.csv`;
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  URL.revokeObjectURL(url);
  toast_('✓ تم تنزيل النسخة الاحتياطية بصيغة Excel');
}

...
def get_season_factor(city="riyadh"):
    today = date.today()
    wd = today.weekday()
    best = 1.0  # الموسم العادي = 1.0 (بدون زيادة)
    ...
    if wd in (3, 4):  # خميس، جمعة
        best = max(best, 1.3)
    # ── المعامل الأدنى قابل للتخصيص من الإعدادات ──
    # الافتراضي 0.78 — لأن مواقع الحجوزات تضع هامش 22% فوق السعر الحقيقي
    # أي السعر الحقيقي للفندق = سعر الموقع × 0.78
    # يمكن تعديله من: الإعدادات ← التسعير ← معامل الموسم الأدنى
    min_factor = float(store.get("settings", {}).get("min_season_factor", 0.78))
    return max(best, min_factor)
  ...
  if(rec&&r.season_factor>=1.0){
    ...
    rec.innerHTML=`
      <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
        <div>📉 سعر المواقع: <b>${F(r.market_adr_ota||0)} ر.س</b> <span style="font-size:10px;opacity:.7">(يشمل عمولة ${ota_pct}%)</span></div>
        <div>✅ سعرك الحقيقي: <b style="color:#25D366">${F(r.market_adr)} ر.س</b></div>
        ${r.season_factor>1?`<div>🔥 موسم ${r.season_label} × ${r.season_factor} = <b>${F(recommended)} ر.س</b></div>`:''}
      </div>`;

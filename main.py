...
def get_market_kpi():
    ...
    # ── معامل خصم عمولة مواقع الحجوزات (افتراضي 0.78 = خصم 22%) ──
    # الأسعار من Booking/Airbnb تشمل عمولة المنصة — نطرحها للحصول على السعر الحقيقي
    ota_factor     = float(s.get("min_season_factor", 0.78))
    market_adr_raw = mr.get(key, 0)
    market_adr     = round(market_adr_raw * ota_factor)   # السعر الحقيقي بعد خصم 22%
    market_adr_ota = market_adr_raw                        # سعر الموقع للعرض فقط
    ...
    recommended_price = round(market_adr * season_factor)

    return {
        "market_adr":        market_adr,
        "market_adr_ota":    market_adr_ota,
        "ota_factor":        ota_factor,
        "ota_commission":    round((1 - ota_factor) * 100, 1),
        "market_occ":        market_occ,
        "market_revpar":     market_revpar,
        "our_adr":           our_adr,
        "our_occ":           our_occ,
        "our_revpar":        our_revpar,
        "adr_diff":          our_adr - market_adr,
        "occ_diff":          our_occ - market_occ,
...
SAUDI_SEASONS = [
    ...
    {"name": "عيد الفطر 1447",  "start": (3,19),  "end": (3,22),  "factor": 2.2},
]
...
def get_season_factor(city="riyadh"):
    ...
    for s in SAUDI_SEASONS:
        ...
        if st <= today <= en:
            if best is None or s["factor"] > best:
                best = s["factor"]
    # ── نهاية الأسبوع = 1.3 (إذا لم يكن موسم أعلى) ──
    if wd in (3, 4):
        best = max(best or 0, 1.3)
    # ── الموسم العادي = 1.0 (بدون زيادة) ──
    if best is None:
        best = 1.0
    return best
...
def get_season_label(city="riyadh"):
    ...
    # اسم المدينة بالعربي
    city_ar = {"riyadh":"الرياض","jeddah":"جدة","makkah":"مكة المكرمة",
               "madinah":"المدينة المنورة","dammam":"الدمام / الخبر",
               "abha":"أبها","taif":"الطائف"}.get(city, city)
    return f"موسم عادي — {city_ar}"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _p_inv_add(self, b):
        ...
        vat  = int(base * 0.15) if b.get("vat", True) else 0   # ريال كامل بدون هللات (floor)
        total = base + vat
        inv = {
    ...
    def _p_cfg_save(self, b):
        s = store.get("settings", {})
        for k in ["name","type","city","rooms","floors","vat_enabled","tourism_tax","mk","ap","recipients","pms_integrations","market_comp_set","owner_iban","owner_bank","owner_account_name","market_price_hotel","market_price_apart","price_hotel_standard","price_hotel_suite","price_apart_studio","price_apart_one","price_apart_two","price_apart_three","ai_price_min_pct","ai_price_max_pct"]:
            if k in b: s[k] = b[k]
        # ── معامل الموسم الأدنى — قابل للتعديل (0.5 إلى 2.0) ──
        if "min_season_factor" in b:
            try:
                v = float(b["min_season_factor"])
                s["min_season_factor"] = max(0.5, min(v, 2.0))
            except:
                s["min_season_factor"] = 1.0
...
{{"hotel_avg":0,"apart_avg":0,"hotel_occ":0,"apart_occ":0,"season":"","season_factor":1.0,"recommendation":"","insight":"","source":"web_search"}}"""
        ...
        try:
            ...
            if claude_key:
                ...
                result = {**base,
                    "season": "موسم عادي",
                    "season_factor": 0.78,
                    "recommendation": f"السعر الموصى به للفندق: {int(base['hotel_avg']*0.78)} ر.س — الشقق: {int(base['apart_avg']*0.78)} ر.س",
                    "insight": "بيانات مدمجة — السعر بعد خصم هامش مواقع الحجوزات (22%). أضف مفتاح Claude API للبحث الحي",
                    "source": "builtin"
                }
    ...
    def _p_market_ai(self, b):
        ...
        typ   = b.get("type", store.get("settings",{}).get("type","hotel"))
        s     = store.get("settings", {})
        # ── نطاق السعر المقبول (قابل للتعديل من الإعدادات) ──
        ai_min_pct = float(s.get("ai_price_min_pct", 65))   # الحد الأدنى 65% من سعر الموقع
        ai_max_pct = float(s.get("ai_price_max_pct", 85))   # الحد الأعلى 85% من سعر الموقع (متوسطه 0.78)
        city_ar = {"riyadh":"الرياض","jeddah":"جدة","makkah":"مكة","madinah":"المدينة المنورة",
                   "dammam":"الدمام","abha":"أبها","taif":"الطائف"}.get(city, city)
        market_ref = store.get("market_rates",{}).get("rates",{}).get(city,{})
        ref_hotel  = market_ref.get("hotel_avg", 0)
        ref_apart  = market_ref.get("apart_avg", 0)
        prompt = f"""أنت خبير في قطاع الفنادق والشقق السعودية.
المدينة: {city_ar} | نوع المنشأة: {"فندق" if typ=="hotel" else "شقق مخدومة"}
اليوم: {datetime.now().strftime("%Y-%m-%d")}
متوسط السوق الحالي (مرجع): فندق {ref_hotel} ر.س / شقق {ref_apart} ر.س
نطاق السعر المقبول: {ai_min_pct}%–{ai_max_pct}% من متوسط السوق

بناءً على معرفتك بالسوق السعودي، قدم:
1. متوسط ADR بالريال للفنادق والشقق في {city_ar} — يجب أن يكون ضمن النطاق المقبول
2. نسبة الإشغال المتوقعة %
3. RevPAR المتوقع
4. الموسم الحالي وتأثيره على التسعير
5. توصية سعرية محددة ضمن النطاق المقبول
...
{{"hotel_avg":0,"apart_avg":0,"hotel_occ":0,"apart_occ":0,"revpar":0,"season":"","season_factor":1.0,"recommendation":"","insight":""}}"""
        try:
            ...
            m = re.search(r'\{.*\}', text, re.DOTALL)
            result = json.loads(m.group(0)) if m else {}
            # ── التحقق من النطاق وتصحيح إذا خرج عن الحدود ──
            if ref_hotel > 0:
                h_min = ref_hotel * ai_min_pct / 100
                h_max = ref_hotel * ai_max_pct / 100
                ai_h  = result.get("hotel_avg", ref_hotel)
                if ai_h < h_min or ai_h > h_max:
                    result["hotel_avg"] = round((h_min + h_max) / 2)
                    result["insight"] = (result.get("insight","") +
                        f" (تم تعديل السعر ليكون ضمن النطاق المقبول {int(h_min)}–{int(h_max)} ر.س)")
            if ref_apart > 0:
                a_min = ref_apart * ai_min_pct / 100
                a_max = ref_apart * ai_max_pct / 100
                ai_a  = result.get("apart_avg", ref_apart)
                if ai_a < a_min or ai_a > a_max:
                    result["apart_avg"] = round((a_min + a_max) / 2)
            # Save to market_rates
            mr = store.get("market_rates", {})
            mr.setdefault("rates", {})[city] = {
                "hotel_avg": result.get("hotel_avg", 0),
            ...
            self._json({"ok": True, "result": result, "source": "claude_ai",
                        "price_range": {"min_pct": ai_min_pct, "max_pct": ai_max_pct}})
        except Exception as e:
...
<body>
<div id="announcement-bar" style="display:none;background:linear-gradient(135deg,#1A56DB,#7C3AED);color:#fff;text-align:center;padding:8px 16px;font-size:12px;font-weight:600;position:fixed;top:0;left:0;right:0;z-index:9999;cursor:pointer" onclick="this.style.display='none'">📢</div>
<div class="app">

<!-- ══ SIDEBAR ══ -->
<div class="sb">
  <div class="sb-logo">
    <div style="font-size:9px;color:rgba(255,255,255,.45);margin-bottom:1px;letter-spacing:.5px" id="sb-sys-name">ضيوف</div>
...
<div class="main">
  <div class="topbar">
    ...
    <div class="tb-r">
      ...
      <button class="btn sm" style="background:#FEE2E2;color:#B91C1C;border:1.5px solid #B91C1C;font-weight:700" onclick="clientLogout()">خروج ↩</button>
    </div>
  </div>
  ...
  <div class="content">
    ...
    <!-- ══════ SUBSCRIPTIONS ══════ -->
      ...
      <div class="kg3" style="margin-bottom:14px">
        <div class="gmc gmc-blue" style="padding:14px"><span class="gm-icon" style="font-size:18px;margin-bottom:6px">💎</span><div class="gm-val" id="sub-plan-name" style="font-size:16px">—</div><div class="gm-lbl">الخطة الحالية</div></div>
        <div class="gmc gmc-green" style="padding:14px"><span class="gm-icon" style="font-size:18px;margin-bottom:6px">📅</span><div class="gm-val" id="sub-exp" style="font-size:14px">—</div><div class="gm-lbl">تاريخ الانتهاء</div></div>
        <div class="gmc gmc-orange" style="padding:14px"><span class="gm-icon" style="font-size:18px;margin-bottom:6px">💳</span><div class="gm-val" id="sub-next-amt" style="font-size:16px">—</div><div class="gm-lbl">القسط الشهري</div></div>
      </div>
      <!-- Dynamic plans from admin -->
      <div id="plans-dynamic-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;margin-bottom:16px">
        <div style="text-align:center;padding:30px;color:var(--muted);font-size:13px">⏳ جارٍ تحميل الباقات...</div>
      </div>
      <!-- Activation -->
      <div class="card cb" style="margin-bottom:12px">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:var(--pl);color:var(--p)">🔑</div>تفعيل الاشتراك بكود</div></div>
        <div class="g3">
          <div class="fg"><label>كود التفعيل</label><input id="sub-code" placeholder="HOTEL-XXXX-XXXX-XXXX" style="font-family:monospace"/></div>
          <div class="fg"><label>الخطة</label><select id="sub-plan-sel"></select></div>
          <div class="fg" style="align-self:flex-end"><button class="btn bp" onclick="activateSub()">تفعيل</button></div>
        </div>
      </div>
      <!-- Bank transfer -->
      <div class="card cg" id="sub-bank-card" style="display:none">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:var(--gl);color:var(--g)">🏦</div>بيانات التحويل البنكي</div></div>
        <div id="sub-bank-info" style="font-size:12px;line-height:1.9"></div>
      </div>
    </div>
...
async function clientLogout(){
  ...
  try{
    await fetch('/api/client/logout',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:'{}'});
  }catch(e){}
...
async function loadSystemInfo(){
  try{
    ...
    const sn=document.getElementById('sb-sys-name');
    if(sn)sn.textContent=d.system_name||'ضيوف';
    document.title=d.system_name||'ضيوف';
    if(d.primary_color){
      document.documentElement.style.setProperty('--p',d.primary_color);
      document.documentElement.style.setProperty('--pl',d.primary_color+'22');
    }
    if(d.announcement){
      const bar=document.getElementById('announcement-bar');
      if(bar){bar.textContent='📢 '+d.announcement;bar.style.display='block';}
    }
    if(d.disabled_pages&&d.disabled_pages.length){
      d.disabled_pages.forEach(pg=>{
        document.querySelectorAll('.ni').forEach(ni=>{
          if(ni.getAttribute('onclick')===`go('${pg}')`)ni.style.display='none';
        });
      });
    }
  }catch(e){}
...
async function boot(){
  ...
  await Promise.all([loadMarket(), loadSystemInfo()]);
  renderUnitTypeGrid();
...
function updateSeasonUI(label,factor){
  const isHigh = factor>=2;
  const isMed  = factor>=1.4;
  const isNorm = factor<=0.3; // موسم عادي
  const bg  = isHigh?'#FEF2F2':isMed?'#FFFBEB':isNorm?'#F0FAF4':'#EFF6FF';
  const col = isHigh?'#991B1B':isMed?'#92400E':isNorm?'#065F46':'#1E40AF';
  ['tb-season','d-season-badge'].forEach(id=>{
    const el=document.getElementById(id); if(!el)return;
    // الموسم العادي: لا يظهر المعامل — فقط اسم الموسم
    if(isNorm){
      el.textContent=`🌿 ${label}`;
    } else {
      el.textContent=`⭐ ${label} × ${factor}`;
    }
...
function renderMarketDash(r){
  ...
  if(rec){
    ...
    const ota_pct = r.ota_commission || 22;
    const isRealSeason = r.season_factor > 1.0;
    const recommended  = r.recommended_price || Math.round(r.market_adr * r.season_factor);
    rec.innerHTML=`
      <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap;font-size:12px">
        <div>📉 سعر المواقع: <b>${F(r.market_adr_ota||0)} ر.س</b>
          <span style="font-size:10px;opacity:.65">(عمولة ${ota_pct}%)</span></div>
        <div>✅ سعرك الحقيقي: <b style="color:#25D366">${F(r.market_adr)} ر.س</b></div>
        ${isRealSeason
          ? `<div>🔥 موسم <b>${r.season_label}</b> × ${r.season_factor} = <b style="color:#DC2626">${F(recommended)} ر.س</b></div>`
          : `<div style="color:#065F46">🌿 موسم عادي — السعر الموصى به: <b>${F(recommended)} ر.س</b></div>`
        }
      </div>`;
  }
}
function renderMarketKPI(r){
  ...
  <div style="padding:10px 12px;background:linear-gradient(135deg,var(--al),#FEF3C7);border-radius:10px;font-size:11px;color:var(--ad);font-weight:700">
    ⭐ السعر الحقيقي للسوق: ${F(r.market_adr)} ر.س (بعد خصم عمولة المواقع ${r.ota_commission||22}%) · الموسم: ${r.season_label} × ${r.season_factor} = السعر الموصى به: <b style="font-size:13px;color:#25D366">${F(r.recommended_price||Math.round(r.market_adr*r.season_factor))} ر.س</b>
  </div>`;
}
...
// ── INVOICES ─────────────────────────────────────────────────
function calcInv(){const base=parseFloat(gv('inv-base')||0),vatOn=document.getElementById('inv-vat')?.checked,vat=vatOn?Math.floor(base*0.15):0;const va=document.getElementById('inv-vat-a');if(va)va.textContent=FR(vat)+' ر.س';const it=document.getElementById('inv-tot');if(it)it.textContent=FR(base+vat)+' ر.س';}
...
// ── SUBSCRIPTIONS ─────────────────────────────────────────────
async function rSubscriptions(){
  const planNames={free:'مجانية',trial:'تجريبية',starter:'المبتدئ',pro:'احترافية',enterprise:'مؤسسية'};
  ss('sub-plan-name',planNames[subState.plan]||subState.plan||'تجريبية');
  ss('sub-exp',subState.expires||'—');
  try{
    const d=await fetch('/api/payment/plans').then(r=>r.json());
    const plans=d.plans||[];
    const bank=d.bank||{};
    const cur=subState.plan||'trial';
    // Current plan amount
    const curP=plans.find(p=>p.id===cur);
    ss('sub-next-amt',(curP?curP.price:0)+' ر.س / شهر');
    // Render plans grid
    const grid=document.getElementById('plans-dynamic-grid');
    if(grid&&plans.length){
      grid.innerHTML=plans.map(p=>{
        const isCur=p.id===cur;
        const col=p.color||'#185FA5';
        const feats=(p.features||[]).map(f=>`<div style="font-size:11px;padding:4px 0;border-bottom:1px solid rgba(0,0,0,.05)">✓ ${f}</div>`).join('');
        const excl=(p.excluded||[]).map(f=>`<div style="font-size:11px;padding:4px 0;border-bottom:1px solid rgba(0,0,0,.05);color:var(--muted);text-decoration:line-through">✕ ${f}</div>`).join('');
        return `<div style="border-radius:14px;border:2px solid ${isCur?col:'var(--border)'};padding:16px;background:${isCur?col+'11':'#fff'};position:relative;transition:.2s" >
          ${p.badge?`<div style="position:absolute;top:-10px;right:50%;transform:translateX(50%);background:${col};color:#fff;font-size:10px;font-weight:700;padding:3px 12px;border-radius:20px">${p.badge}</div>`:''}
          ${isCur?'<div style="position:absolute;top:8px;left:8px;background:'+col+';color:#fff;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px">✓ خطتك</div>':''}
          <div style="font-size:15px;font-weight:800;color:${col};margin:${p.badge?'10':'0'}px 0 4px">${p.name}</div>
          <div style="font-size:26px;font-weight:900;color:var(--text);margin-bottom:4px">${p.price}<span style="font-size:12px;font-weight:500;color:var(--muted)"> ر.س/${p.period||'شهر'}</span></div>
          ${p.pos_devices!==undefined?`<div style="font-size:10px;color:var(--muted);margin-bottom:10px">🖥️ ${p.pos_devices===0||p.pos_devices==='unlimited'?'غير محدود':p.pos_devices} جهاز POS</div>`:''}
          <div style="margin-bottom:14px">${feats}${excl}</div>
          <button class="btn sm" style="width:100%;background:${isCur?col:'transparent'};color:${isCur?'#fff':col};border:2px solid ${col}" onclick="selectPlanDyn('${p.id}','${p.name}',${p.price},'${col}')">${isCur?'خطتك الحالية ✓':'اختيار هذه الخطة'}</button>
        </div>`;
      }).join('');
    } else if(grid){
      grid.innerHTML='<div style="padding:20px;text-align:center;color:var(--muted)">تواصل مع المدير لعرض الباقات</div>';
    }
    // Plan selector
    const sel=document.getElementById('sub-plan-sel');
    if(sel)sel.innerHTML=plans.map(p=>`<option value="${p.id}">${p.name} — ${p.price} ر.س/شهر</option>`).join('');
    // Bank info
    const bc=document.getElementById('sub-bank-card');
    const bi=document.getElementById('sub-bank-info');
    if(bank.iban&&bc&&bi){
      bc.style.display='block';
      bi.innerHTML=`<div>اسم الحساب: <b>${bank.name||'—'}</b></div><div>البنك: <b>${bank.bank||'—'}</b></div><div style="font-family:monospace;font-size:13px;font-weight:700;color:var(--gd);margin-top:4px">IBAN: ${bank.iban}</div>${bank.whatsapp?`<div style="margin-top:6px"><a href="https://wa.me/${bank.whatsapp}" target="_blank" style="color:var(--g);font-weight:600">📱 واتساب: ${bank.whatsapp}</a></div>`:''}`;
    }
  }catch(e){console.error('rSubscriptions',e);}
}
async function selectPlanDyn(id,name,price,color){
  if(id===subState.plan){toast_('أنت على هذه الخطة بالفعل ✓');return;}
  const sel=document.getElementById('sub-plan-sel');
  if(sel)for(let i=0;i<sel.options.length;i++)if(sel.options[i].value===id){sel.selectedIndex=i;break;}
  try{
    const r=await api('/api/client/select-plan',{plan_id:id,plan_name:name,price});
    const bc=document.getElementById('sub-bank-card');
    const bi=document.getElementById('sub-bank-info');
    const bk=(r&&r.bank)||{};
    if(bk.iban&&bc&&bi){
      bc.style.display='block';
      bi.innerHTML=`
        <div style="padding:4px 0">اسم الحساب: <b>${bk.name||'—'}</b></div>
        <div style="padding:4px 0">البنك: <b>${bk.bank||'—'}</b></div>
        <div style="font-family:monospace;font-size:15px;font-weight:700;padding:8px 0;letter-spacing:1px;color:#185FA5">IBAN: ${bk.iban}</div>
        ${bk.whatsapp?`<a href="https://wa.me/${bk.whatsapp}" target="_blank" style="display:inline-block;margin-top:6px;color:#25D366;font-weight:600">📱 واتساب: ${bk.whatsapp}</a>`:''}
        ${bk.phone?`<div style="margin-top:4px;font-size:13px">📞 هاتف: ${bk.phone}</div>`:''}
        <div style="margin-top:10px;padding:8px;background:rgba(255,200,0,.15);border-radius:8px;font-size:12px;line-height:1.7">
          ⚠️ بعد التحويل أرسل صورة الإيصال عبر واتساب — سيصلك كود التفعيل خلال 24 ساعة.
        </div>`;
      bc.scrollIntoView({behavior:'smooth',block:'center'});
      toast_('✅ تم إرسال طلبك — راجع بيانات التحويل أدناه');
    } else {
      toast_(`لتفعيل باقة ${name} (${price} ر.س) — أدخل كود التفعيل أدناه`);
    }
  }catch(e){
    toast_(`لتفعيل باقة ${name} (${price} ر.س) — أدخل كود التفعيل أدناه`);
  }
...
function activateSub(){
  ...
  const planSel=document.getElementById('sub-plan-sel');
  const plan=planSel?planSel.value:'pro';
  const planName=planSel&&planSel.selectedIndex>=0?planSel.options[planSel.selectedIndex].text:plan;
  if(!code){toast_('أدخل كود التفعيل',false);return;}
  if(!code.startsWith('HOTEL-')){toast_('الكود يجب أن يبدأ بـ HOTEL-',false);return;}
  api('/api/client/activate',{key:code}).then(r=>{
    if(r.ok){
      subState.plan=plan;
      const exp=new Date();exp.setFullYear(exp.getFullYear()+1);
      subState.expires=exp.toLocaleDateString('ar-SA');
      rSubscriptions();
      toast_('تم تفعيل '+planName+' ✓');
    } else {
      toast_(r.error||'كود غير صحيح',false);
    }
  }).catch(()=>toast_('خطأ في الاتصال',false));
}
...
function buildReport(){
  ...
  const iban=S.settings.bank_iban||S.settings.owner_iban||'';
  const bank=S.settings.bank_name||S.settings.owner_bank||'';
  const accName=S.settings.owner_account_name||S.settings.owner_name||'';
  ...
  <div style="font-family:'Tajawal',system-ui,Arial;direction:rtl;color:#0F172A">

    <!-- HEADER -->
    <div style="background:linear-gradient(135deg,#0A3D2E,#1A6B4A);border-radius:12px;padding:18px 22px;margin-bottom:14px;color:#fff">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px">
        <div>
          <div style="font-size:20px;font-weight:800">${S.settings.name||'الفندق'}</div>
          <div style="font-size:12px;opacity:.7;margin-top:3px">${NOW.toLocaleDateString('ar-SA',{weekday:'long',year:'numeric',month:'long',day:'numeric'})}</div>
          <div style="font-size:11px;opacity:.5;margin-top:2px">يُرسل إلى: ${(S.settings.recipients||[]).map(r=>r.name).join(' · ')||'—'}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px">
          ${matchResult?`<span style="font-size:11px;padding:5px 14px;border-radius:20px;font-weight:700;background:${matchResult.stC}30;color:${matchResult.stC}">${matchResult.stL}</span>`:''}
          <button class="btn bp sm" onclick="downloadReport()" style="background:rgba(255,255,255,.15);border-color:rgba(255,255,255,.3);color:#fff">⬇️ تنزيل</button>
        </div>
      </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px">
      <div style="background:#ECFDF5;border:1px solid #6EE7B7;border-radius:10px;padding:14px;text-align:center">
        <div style="font-size:11px;color:#065F46;font-weight:600;margin-bottom:4px">💰 إجمالي الإيرادات</div>
        <div style="font-size:22px;font-weight:800;color:#047857">${FR(si)}</div>
      </div>
      <div style="background:#FEF2F2;border:1px solid #FCA5A5;border-radius:10px;padding:14px;text-align:center">
        <div style="font-size:11px;color:#991B1B;font-weight:600;margin-bottom:4px">📤 إجمالي المصاريف</div>
        <div style="font-size:22px;font-weight:800;color:#DC2626">${FR(so)}</div>
      </div>
      <div style="background:${net>=0?'#EFF6FF':'#FEF2F2'};border:1px solid ${net>=0?'#BFDBFE':'#FCA5A5'};border-radius:10px;padding:14px;text-align:center">
        <div style="font-size:11px;color:${net>=0?'#1E40AF':'#991B1B'};font-weight:600;margin-bottom:4px">${net>=0?'✅':'⚠️'} الصافي النهائي</div>
        <div style="font-size:22px;font-weight:800;color:${net>=0?'#1A56DB':'#DC2626'}">${FR(net)}</div>
      </div>
    </div>
    ...
    <div style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;margin-bottom:10px;overflow:hidden">
      <div style="background:#F8FAFC;padding:10px 16px;border-bottom:1px solid #E2E8F0;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:13px;font-weight:700">👥 النزلاء</span>
        <span style="font-size:11px;color:#64748B">${S.guests.length} إجمالي · ${S.guests.filter(g=>g.status==='active').length} مقيم · ${S.guests.filter(g=>g.status!=='active').length} مغادر</span>
      </div>
      ${S.guests.length?`
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead><tr style="background:#F1F5F9">
          <th style="padding:7px 12px;text-align:right;color:#475569;font-weight:600">الاسم</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">الوحدة</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">الليالي</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">طريقة الدفع</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">الإيجار</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">الحالة</th>
        </tr></thead>
        <tbody>${S.guests.map((g,i)=>`
        <tr style="border-bottom:1px solid #F1F5F9;background:${i%2?'#FAFAFA':'#fff'}">
          <td style="padding:8px 12px;font-weight:600">${g.name}</td>
          <td style="padding:8px;text-align:center;color:#64748B">${g.unit||'—'}</td>
          <td style="padding:8px;text-align:center">${g.nights||1}</td>
          <td style="padding:8px;text-align:center;color:#64748B;font-size:10px">${g.payMethod||'—'}</td>
          <td style="padding:8px;text-align:right;font-weight:700;color:#047857">${FR(g.total)}</td>
          <td style="padding:8px;text-align:center"><span style="font-size:10px;padding:2px 8px;border-radius:99px;font-weight:600;background:${g.status==='active'?'#ECFDF5':'#F1F5F9'};color:${g.status==='active'?'#065F46':'#475569'}">${g.status==='active'?'مقيم':'مغادر'}</span></td>
        </tr>`).join('')}
        </tbody>
        <tfoot><tr style="background:#F8FAFC;border-top:2px solid #E2E8F0">
          <td colspan="4" style="padding:8px 12px;font-weight:700;font-size:12px">إجمالي إيرادات النزلاء</td>
          <td style="padding:8px;text-align:right;font-weight:800;color:#047857;font-size:13px">${FR(S.guests.reduce((a,g)=>a+g.total,0))}</td>
          <td></td>
        </tr></tfoot>
      </table>`:'<div style="padding:16px;text-align:center;color:#94A3B8;font-size:12px">لا يوجد نزلاء اليوم</div>'}
    </div>

    <!-- أجهزة نقاط البيع POS — تفصيلي بالقسم -->
    ${S.pos_devices.length?`
    <div style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;margin-bottom:10px;overflow:hidden">
      <div style="background:#F8FAFC;padding:10px 16px;border-bottom:1px solid #E2E8F0;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:13px;font-weight:700">💳 إيرادات نقاط البيع — تفصيل بالقسم</span>
        <span style="font-size:12px;font-weight:800;color:#047857">${FR(pNet)} إجمالي</span>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead><tr style="background:#F1F5F9">
          <th style="padding:7px 12px;text-align:right;color:#475569;font-weight:600">القسم / الجهاز</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">إجمالي المبيعات</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">مسترد</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">الصافي</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">نقداً</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">بطاقات</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">معاملات</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">الحالة</th>
        </tr></thead>
        <tbody>${S.pos_devices.map((d,i)=>{
          const dNet=d.data?d.data.sales-(d.data.refund||0):0;
          const cards=d.data?((d.data.breakdown.mada||0)+(d.data.breakdown.visa||0)+(d.data.breakdown.mastercard||0)+(d.data.breakdown.stcpay||0)+(d.data.breakdown.applepay||0)):0;
          const icon=POS_ICONS[d.dept]||'💳';
          const dept=POS_DEPT[d.dept]||d.dept;
          return`<tr style="border-bottom:1px solid #F1F5F9;background:${i%2?'#FAFAFA':'#fff'}">
            <td style="padding:8px 12px">
              <div style="font-weight:700;font-size:12px">${icon} إيراد ${dept}</div>
              <div style="font-size:10px;color:#94A3B8;margin-top:1px">${d.name}</div>
            </td>
            <td style="padding:8px;text-align:right;color:#047857;font-weight:700">${d.data?FR(d.data.sales):'—'}</td>
            <td style="padding:8px;text-align:right;color:${d.data&&d.data.refund?'#DC2626':'#94A3B8'}">${d.data&&d.data.refund?'('+FR(d.data.refund)+')':'—'}</td>
            <td style="padding:8px;text-align:right;font-weight:800;color:${dNet>=0?'#1A56DB':'#DC2626'}">${d.data?FR(dNet):'—'}</td>
            <td style="padding:8px;text-align:right;color:#475569">${d.data?FR(d.data.breakdown.cash||0):'—'}</td>
            <td style="padding:8px;text-align:right;color:#475569">${d.data?FR(cards):'—'}</td>
            <td style="padding:8px;text-align:center;color:#64748B">${d.data?d.data.txCount||0:'—'}</td>
            <td style="padding:8px;text-align:center"><span style="font-size:10px;padding:2px 8px;border-radius:99px;font-weight:600;background:${d.data?'#ECFDF5':'#FEF9C3'};color:${d.data?'#065F46':'#854D0E'}">${d.data?'مُستلم ✓':'انتظار'}</span></td>
          </tr>`;}).join('')}
        </tbody>
        <tfoot><tr style="background:#F0FDF4;border-top:2px solid #6EE7B7">
          <td style="padding:8px 12px;font-weight:800;color:#047857">الإجمالي</td>
          <td style="padding:8px;text-align:right;font-weight:800;color:#047857">${FR(S.pos_devices.filter(d=>d.data).reduce((a,d)=>a+d.data.sales,0))}</td>
          <td style="padding:8px;text-align:right;color:#DC2626;font-weight:700">(${FR(S.pos_devices.filter(d=>d.data).reduce((a,d)=>a+(d.data.refund||0),0))})</td>
          <td style="padding:8px;text-align:right;font-weight:800;color:#1A56DB;font-size:13px">${FR(pNet)}</td>
          <td colspan="4"></td>
        </tr></tfoot>
      </table>
    </div>`:''}
    ...
    ${S.services.length?`
    <div style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;margin-bottom:10px;overflow:hidden">
      <div style="background:#F8FAFC;padding:10px 16px;border-bottom:1px solid #E2E8F0;display:flex;justify-content:space-between">
        <span style="font-size:13px;font-weight:700">🛎️ الخدمات الإضافية</span>
        <span style="font-size:12px;font-weight:700;color:#7C3AED">${FR(S.services.reduce((a,s)=>a+s.amount,0))}</span>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead><tr style="background:#F1F5F9">
          <th style="padding:7px 12px;text-align:right;color:#475569;font-weight:600">الخدمة</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">النزيل</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">المبلغ</th>
        </tr></thead>
        <tbody>${S.services.map((s,i)=>`
        <tr style="border-bottom:1px solid #F1F5F9;background:${i%2?'#FAFAFA':'#fff'}">
          <td style="padding:7px 12px;font-weight:600">${s.name||'خدمة'}</td>
          <td style="padding:7px 8px;color:#64748B">${s.guestName||'—'}</td>
          <td style="padding:7px 8px;font-weight:700;color:#7C3AED">${FR(s.amount)}</td>
        </tr>`).join('')}
        </tbody>
      </table>
    </div>`:''}
    ...
    <!-- فواتير الموردين -->
    ${S.invoices.filter(i=>i.status==='pending').length?`
    <div style="background:#fff;border:1px solid #FCA5A5;border-radius:10px;margin-bottom:10px;overflow:hidden">
      <div style="background:#FEF2F2;padding:10px 16px;border-bottom:1px solid #FCA5A5;display:flex;justify-content:space-between">
        <span style="font-size:13px;font-weight:700;color:#991B1B">📋 فواتير موردين معلقة</span>
        <span style="font-size:12px;font-weight:800;color:#DC2626">${FR(S.invoices.filter(i=>i.status==='pending').reduce((a,i)=>a+i.total,0))}</span>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:11px">
        <thead><tr style="background:#FFF5F5">
          <th style="padding:7px 12px;text-align:right;color:#475569;font-weight:600">المورّد</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">رقم الفاتورة</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">الأساسي</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">VAT 15%</th>
          <th style="padding:7px 8px;text-align:right;color:#475569;font-weight:600">الإجمالي</th>
          <th style="padding:7px 8px;text-align:center;color:#475569;font-weight:600">الاستحقاق</th>
        </tr></thead>
        <tbody>${S.invoices.filter(i=>i.status==='pending').map((inv,idx)=>`
        <tr style="border-bottom:1px solid #FEE2E2;background:${idx%2?'#FFF8F8':'#fff'}">
          <td style="padding:7px 12px;font-weight:700">${inv.supName}</td>
          <td style="padding:7px 8px;text-align:center;color:#64748B;font-size:10px">${inv.num}</td>
          <td style="padding:7px 8px">${FR(inv.base)}</td>
          <td style="padding:7px 8px;color:#D97706">${inv.vat>0?FR(inv.vat):'—'}</td>
          <td style="padding:7px 8px;font-weight:700;color:#DC2626">${FR(inv.total)}</td>
          <td style="padding:7px 8px;text-align:center;font-size:10px;color:#64748B">${inv.due||'—'}</td>
        </tr>`).join('')}
        </tbody>
      </table>
    </div>`:''}
    ...
    <!-- مقارنة السوق -->
    ${marketData&&marketData.our_adr?`
    <div style="background:#fff;border:1px solid #BFDBFE;border-radius:10px;margin-bottom:10px;overflow:hidden">
      <div style="background:#EFF6FF;padding:10px 16px;border-bottom:1px solid #BFDBFE">
        <span style="font-size:13px;font-weight:700;color:#1E40AF">📊 مقارنة أداء السوق</span>
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr)">
        ${[['ADR — متوسط السعر','our_adr','market_adr',' ر.س'],['نسبة الإشغال','our_occ','market_occ','%'],['RevPAR','our_revpar','market_revpar',' ر.س']].map(([lbl,ok,mk,unit])=>`
        <div style="padding:12px 14px;border-left:1px solid #E2E8F0;text-align:center">
          <div style="font-size:10px;color:#64748B;margin-bottom:5px">${lbl}</div>
          <div style="font-size:16px;font-weight:800;color:#1A56DB">${unit==='%'?marketData[ok]+'%':FR(marketData[ok])}</div>
          <div style="font-size:10px;color:#94A3B8;margin-top:3px">السوق: ${unit==='%'?marketData[mk]+'%':FR(marketData[mk])}</div>
          <div style="font-size:11px;font-weight:700;color:${marketData[ok]>=marketData[mk]?'#065F46':'#DC2626'};margin-top:3px">${marketData[ok]>=marketData[mk]?'↑ أعلى من السوق':'↓ أقل من السوق'}</div>
        </div>`).join('')}
      </div>
      <div style="padding:9px 14px;font-size:12px;color:#1E40AF;background:#EFF6FF;border-top:1px solid #BFDBFE">
        ⭐ الموسم: <b>${marketData.season_label}</b>${marketData.season_factor!==1?` × ${marketData.season_factor}`:''} · السعر الموصى به: <b>${FR(marketData.recommended_price||0)}</b>
      </div>
    </div>`:''}
    ...
    <!-- FOOTER -->
    <div style="margin-top:12px;padding:10px 16px;background:#F8FAFC;border-radius:8px;border:1px solid #E2E8F0;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">
      <div style="font-size:10px;color:#94A3B8">تقرير تلقائي — برنامج ضيوف v3.0</div>
      <div style="font-size:10px;color:#94A3B8">${TNOW()}</div>
    </div>
  </div>`;
}
...
body{font-family:'Tajawal',system-ui,Arial;direction:rtl;padding:28px;background:#fff;color:#0F172A;max-width:820px;margin:0 auto;font-size:13px}
...
@media print{body{padding:16px}button{display:none!important}}
  ...
  a.download=`تقرير_${S.settings.name||'الفندق'}_${new Date().toISOString().slice(0,10)}.html`;
...
async function runWebSearchMarket(){
  ...
  if(badge){badge.style.display='block';badge.innerHTML='<span style="font-size:10px;color:var(--muted)">🔍 يبحث في Booking وAgoda وGoogle Hotels...</span>';}
  ...
  if(r.ok&&r.result){
    ...
    // ── Rate limited — عرض نتيجة الكاش مع رسالة الانتظار
    if(r.cached&&res.rate_limited){
      if(badge)badge.innerHTML=`<span style="font-size:10px;padding:3px 9px;border-radius:20px;background:#FFFBEB;color:#92400E;font-weight:700">⏱ بحث مخزّن — بعد ${res.wait_minutes} دقيقة يمكن التحديث</span>`;
      toast_(`آخر بحث قبل ${60-res.wait_minutes} دقيقة — التحديث بعد ${res.wait_minutes} دقيقة`,false);
    } else {
      if(badge)badge.innerHTML=`<span style="font-size:10px;padding:3px 9px;border-radius:20px;background:${res.source&&res.source.includes('Claude')?'#ECFDF5':'#EEF2FF'};color:${res.source&&res.source.includes('Claude')?'#065F46':'#3730A3'};font-weight:700">${res.source&&res.source.includes('Claude')?'✅ بحث حي + Claude AI':'📊 بيانات مرجعية'}</span>`;
      toast_('✅ تم تحديث أسعار السوق');
    }

    // تحديث بيانات السوق المحلية
    S.market_rates=S.market_rates||{};S.market_rates.rates=S.market_rates.rates||{};
    ...
    // بناء عرض المواقع
    const bk=res.booking_samples||[];const ag=res.agoda_samples||[];const gl=res.google_samples||[];
    const siteRow=(arr,logo,color)=>arr.length?`
      <div style="margin-top:8px">
        <div style="font-size:10px;font-weight:700;color:${color};margin-bottom:4px">${logo}</div>
        <div style="display:flex;flex-direction:column;gap:3px">
          ${arr.slice(0,3).map(h=>`<div style="display:flex;justify-content:space-between;align-items:center;background:rgba(255,255,255,.7);border-radius:6px;padding:4px 8px;font-size:10px">
            <span style="font-weight:600">${h.name||'—'}</span>
            <span style="display:flex;align-items:center;gap:6px">
              ${'⭐'.repeat(Math.min(h.stars||0,5))}
              ${h.rating?`<span style="color:#D97706">${h.rating}</span>`:''}
              <b style="color:#185FA5">${F(h.price||0)} ر.س</b>
            </span>
          </div>`).join('')}
        </div>
      </div>`:'';
    ...
    if(el){
      ...
      el.innerHTML=`
        <div class="ab-t">🔍 أسعار السوق الحية — ${res.city_ar||res.city||''} ${res.checkin||''}</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:8px 0;text-align:center">
          <div style="background:rgba(26,86,219,.08);border-radius:8px;padding:8px">
            <div style="font-size:9px;color:var(--muted);font-weight:700">فنادق ADR</div>
            <div style="font-size:18px;font-weight:900;color:var(--p)">${F(res.hotel_avg||0)}</div>
            <div style="font-size:9px;color:var(--muted)">ر.س / ليلة</div>
          </div>
          <div style="background:rgba(124,58,237,.08);border-radius:8px;padding:8px">
            <div style="font-size:9px;color:var(--muted);font-weight:700">شقق ADR</div>
            <div style="font-size:18px;font-weight:900;color:var(--u)">${F(res.apart_avg||0)}</div>
            <div style="font-size:9px;color:var(--muted)">ر.س / ليلة</div>
          </div>
          <div style="background:rgba(4,120,87,.08);border-radius:8px;padding:8px">
            <div style="font-size:9px;color:var(--muted);font-weight:700">إشغال</div>
            <div style="font-size:18px;font-weight:900;color:var(--g)">${res.hotel_occ||0}%</div>
            <div style="font-size:9px;color:var(--muted)">${res.season||'—'} ×${res.season_factor||1}</div>
          </div>
        </div>
        ${res.booking_avg||res.agoda_avg||res.google_avg?`
        <div style="display:flex;gap:8px;margin-bottom:6px;flex-wrap:wrap">
          ${res.booking_avg?`<span style="font-size:10px;padding:3px 8px;border-radius:6px;background:#003580;color:#fff;font-weight:700">Booking: ${F(res.booking_avg)} ر.س</span>`:''}
          ${res.agoda_avg?`<span style="font-size:10px;padding:3px 8px;border-radius:6px;background:#e0131f;color:#fff;font-weight:700">Agoda: ${F(res.agoda_avg)} ر.س</span>`:''}
          ${res.google_avg?`<span style="font-size:10px;padding:3px 8px;border-radius:6px;background:#4285F4;color:#fff;font-weight:700">Google: ${F(res.google_avg)} ر.س</span>`:''}
        </div>`:''}
        ${siteRow(bk,'🔵 Booking.com','#003580')}
        ${siteRow(ag,'🔴 Agoda','#e0131f')}
        ${siteRow(gl,'🔷 Google Hotels','#4285F4')}
        ${res.recommendation?`<div style="margin-top:8px;padding:8px 10px;background:#F0FDF4;border-radius:8px;font-size:11px;color:#065F46;font-weight:600">💡 ${res.recommendation}</div>`:''}
        ${res.insight?`<div style="margin-top:4px;font-size:10px;color:var(--muted)">${res.insight}</div>`:''}
        <div style="margin-top:6px;font-size:9px;color:var(--muted);text-align:left">المصدر: ${res.source||'—'} — ${res.searched_at?new Date(res.searched_at).toLocaleTimeString('ar-SA'):''}</div>`;
    }
  } else {
    if(badge)badge.innerHTML=`<span style="font-size:10px;color:var(--r)">${r.error||'خطأ في البحث'}</span>`;
    toast_('تعذّر البحث — تحقق من الاتصال',false);
  }
}

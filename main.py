...
#  HTTP Handler
# ══════════════════════════════════════════════════════════════
def _get_owner_ai_key():
    """يجلب مفتاح Claude API للمالك من ملف admin.json المشترك"""
    try:
        import os, json as _json
        base = os.path.dirname(os.path.abspath(__file__))
        data_file = os.path.join(base, "..", "HotelAdmin", "data", "admin.json")
        if not os.path.exists(data_file):
            data_file = os.path.join(base, "HotelAdmin", "data", "admin.json")
        if os.path.exists(data_file):
            data = _json.loads(open(data_file, encoding="utf-8").read())
            return data.get("admin_settings", {}).get("claude_key", "")
    except Exception:
        pass
    return ""
...
نظام إدارة الفندق v3.0"""
    ...
    def _p_market_websearch(self, b):
        ...
        claude_key = store.get("settings",{}).get("claude_key","") or _get_owner_ai_key()
        ...
        try:
            ...
            req = ur.Request(
                ...
                headers={
                    "Content-Type": "application/json",
                    "anthropic-version": "2023-06-01",
                    "x-api-key": (store.get("settings",{}).get("claude_key","") or _get_owner_ai_key()),
                },
                method="POST"
            )
...
<!-- Announcement bar (shown when admin sets announcement) -->
<div id="announcement-bar" style="display:none;background:linear-gradient(135deg,#1A56DB,#7C3AED);color:#fff;text-align:center;padding:8px 16px;font-size:12px;font-weight:600;position:fixed;top:0;left:0;right:0;z-index:999;cursor:pointer" onclick="document.getElementById('announcement-bar').style.display='none'">
  انقر للإغلاق ✕
</div>
...
<div class="sb">
  <div class="sb-logo">
    <div style="font-size:9px;color:rgba(255,255,255,.4);margin-bottom:2px;letter-spacing:.5px" id="sb-sys-name">نظام إدارة الفنادق</div>
    ...
    <div style="display:flex;align-items:center;gap:6px;margin-top:4px">
      ...
      <div style="position:relative;cursor:pointer;margin-right:auto" onclick="toggleNotifPanel()">
        <span style="font-size:14px">🔔</span>
        <span id="notif-badge" style="display:none;position:absolute;top:-4px;right:-4px;background:#EF4444;color:#fff;font-size:8px;font-weight:700;border-radius:50%;width:14px;height:14px;align-items:center;justify-content:center"></span>
      </div>
    </div>
  </div>
  <!-- Notification panel -->
  <div id="notif-panel" style="display:none;background:rgba(0,0,0,.3);border-radius:10px;margin:0 8px 8px;max-height:200px;overflow-y:auto">
    <div style="padding:8px 12px;font-size:10px;font-weight:700;color:rgba(255,255,255,.6);border-bottom:1px solid rgba(255,255,255,.1)">📢 إشعارات من الإدارة</div>
    <div id="notif-list" style="font-size:11px;color:rgba(255,255,255,.8)"></div>
  </div>
  ...
  <div class="ni" onclick="go('ai')"><span class="ni-ic">🤖</span>مساعد AI</div>
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="pg" id="pg-ai">
      <!-- Header -->
      <div style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);border-radius:14px;padding:18px 20px;margin-bottom:14px;color:#fff;display:flex;align-items:center;gap:14px">
        <div style="font-size:36px">🤖</div>
        <div>
          <div style="font-size:16px;font-weight:700;margin-bottom:4px">مساعد الذكاء الاصطناعي</div>
          <div id="ai-status-line" style="font-size:11px;opacity:.8">جاري التحقق من المفتاح...</div>
        </div>
        <div id="ai-key-badge" style="margin-right:auto;padding:5px 12px;border-radius:20px;font-size:11px;font-weight:600;background:rgba(255,255,255,.15)">—</div>
      </div>

      <!-- Card 1: أسعار السوق -->
      <div class="card cb" style="margin-bottom:12px">
        <div class="ch">
          <div class="ch-l"><div class="cico" style="background:var(--ul);color:var(--u)">🌐</div>تحديث أسعار السوق</div>
        </div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:12px">يبحث Claude في الإنترنت عن أسعار الفنادق الفعلية في مدينتك الآن ثم يحدّث بياناتك تلقائياً.</div>
        <div class="g3" style="margin-bottom:12px">
          <div class="fg"><label>المدينة</label>
            <select id="ai-mkt-city">
              <option value="riyadh">الرياض</option><option value="jeddah">جدة</option>
              <option value="makkah">مكة المكرمة</option><option value="madinah">المدينة المنورة</option>
              <option value="dammam">الدمام</option><option value="khobar">الخبر</option>
              <option value="jubail">الجبيل</option><option value="ahsa">الأحساء</option>
              <option value="abha">أبها</option><option value="taif">الطائف</option>
              <option value="tabuk">تبوك</option><option value="qassim">القصيم</option>
              <option value="hail">حائل</option><option value="bahah">الباحة</option>
              <option value="other">أخرى</option>
            </select>
          </div>
          <div class="fg"><label>نوع المنشأة</label>
            <select id="ai-mkt-type"><option value="hotel">فندق</option><option value="apart">شقق مخدومة</option></select>
          </div>
          <div class="fg" style="align-self:flex-end">
            <button class="btn bu2" id="ai-mkt-btn" onclick="aiUpdateMarket()">🔍 بحث ويب + AI</button>
          </div>
        </div>
        <div id="ai-mkt-result" style="display:none;background:var(--gl);border-radius:10px;padding:12px;font-size:11px"></div>
      </div>

      <!-- Card 2: تحديث الموسم -->
      <div class="card ca" style="margin-bottom:12px">
        <div class="ch">
          <div class="ch-l"><div class="cico" style="background:var(--al);color:var(--a)">🌙</div>تحليل الموسم السياحي</div>
        </div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:12px">يحلل Claude الموسم الحالي ويقترح معامل تسعير مناسب بناءً على مدينتك والتاريخ الحالي.</div>
        <div class="g3" style="margin-bottom:12px">
          <div class="fg"><label>المدينة</label>
            <select id="ai-season-city">
              <option value="riyadh">الرياض</option><option value="jeddah">جدة</option>
              <option value="makkah">مكة المكرمة</option><option value="madinah">المدينة المنورة</option>
              <option value="dammam">الدمام</option><option value="khobar">الخبر</option>
              <option value="abha">أبها</option><option value="taif">الطائف</option>
              <option value="tabuk">تبوك</option><option value="other">أخرى</option>
            </select>
          </div>
          <div class="fg" style="align-self:flex-end">
            <button class="btn ba2" id="ai-season-btn" onclick="aiAnalyzeSeason()">🌙 تحليل الموسم</button>
          </div>
        </div>
        <div id="ai-season-result" style="display:none;background:var(--al);border-radius:10px;padding:12px;font-size:11px"></div>
      </div>

      <!-- Card 3: تحليل البيانات -->
      <div class="card cg" style="margin-bottom:12px">
        <div class="ch">
          <div class="ch-l"><div class="cico" style="background:var(--gl);color:var(--g)">📊</div>تحليل بيانات الفندق</div>
        </div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:12px">يحلل Claude بيانات فندقك الحالية (النزلاء، الإيرادات، الإشغال) ويعطيك توصيات عملية.</div>
        <button class="btn bg2" id="ai-data-btn" onclick="aiAnalyzeData()" style="margin-bottom:12px">📊 تحليل بياناتي الآن</button>
        <div id="ai-data-result" style="display:none;background:var(--gl);border-radius:10px;padding:14px;font-size:11px;line-height:1.7;white-space:pre-wrap"></div>
      </div>

      <!-- No key warning -->
      <div id="ai-no-key-warn" style="display:none;background:var(--rl);border:1.5px solid var(--r);border-radius:10px;padding:14px;font-size:12px;color:var(--rd);text-align:center">
        ⚠️ لا يوجد مفتاح Claude API — تواصل مع مدير النظام لتفعيل الذكاء الاصطناعي
      </div>
    </div>
    ...
    <div class="pg" id="pg-subscriptions">
      ...
      <div class="kg3" style="margin-bottom:14px">
        ...
        <div class="gmc gmc-orange" style="padding:14px"><span class="gm-icon" style="font-size:18px;margin-bottom:6px">💳</span><div class="gm-val" id="sub-next-amt" style="font-size:16px">—</div><div class="gm-lbl">القسط القادم</div></div>
      </div>

      <!-- Dynamic plans grid from admin -->
      ...
      <div class="card cb" style="margin-bottom:12px">
        ...
        <div style="font-size:11px;color:var(--muted);margin-bottom:10px">أدخل كود التفعيل الذي أرسله لك مدير النظام</div>
        <div class="g3">
          ...
          <div class="fg"><label>الخطة</label><select id="sub-plan-sel" style="font-family:inherit"></select></div>
        </div>
...
const PAGE_META={
  ...
  ai:{ic:'🤖',bg:'linear-gradient(135deg,#1a1a2e,#0f3460)',col:'#fff',title:'مساعد الذكاء الاصطناعي'},
...
let SYS={system_name:'نظام إدارة الفندق',owner_whatsapp:'',owner_phone:'',contact_message:'',trial_days:15,disabled_pages:[],announcement:'',logo_emoji:'🏨',primary_color:'#185FA5'};

async function loadSystemInfo(){
  try{
    const d=await fetch('/api/system-info').then(r=>r.json());
    Object.assign(SYS,d);
    // Apply system name to sidebar
    const sn=document.getElementById('sb-sys-name');if(sn)sn.textContent=d.system_name||SYS.system_name;
    document.title=d.system_name||SYS.system_name;
    // Apply primary color
    if(d.primary_color){
    ...
    if(d.disabled_pages&&d.disabled_pages.length){
      d.disabled_pages.forEach(pg=>{
        const ni=document.querySelector(`.ni[onclick="go('${pg}')"]`);
        if(ni)ni.style.display='none';
        const pgEl=document.getElementById('pg-'+pg);
        if(pgEl)pgEl.innerHTML='<div style="padding:40px;text-align:center;color:var(--muted)">🔒 هذه الصفحة غير متاحة في خطتك الحالية</div>';
      });
    }
  }catch(e){}
}
...
async function loadClientNotifications(){
  try{
    ...
    if(list){
      list.innerHTML=notifs.map(n=>`<div style="padding:10px 12px;border-bottom:1px solid var(--border);font-size:12px">
        <div style="font-weight:600;margin-bottom:2px">${n.message||''}</div>
        <div style="font-size:10px;color:var(--muted)">${n.from||'الإدارة'} · ${n.time?new Date(n.time).toLocaleString('ar-SA'):''}</div>
      </div>`).join('');
    }
  }catch(e){}
}
...
async function boot(){
  ...
  await Promise.all([loadMarket(), loadSystemInfo(), loadClientNotifications()]);
  renderUnitTypeGrid();
...
function go(pg){
  ...
  ss('tb-title',m.title);
  const rs={dash:rDash,kpi:rKPI,ai:initAIPage,pms:rPMS,guests:rGuests,support:rSupport,svcs:rSvcs,pos:rPOS,journal:rJnl,trial:rTrial,pl:rPL,recv:rRecv,paybl:rPaybl,suppliers:rSup,invoices:rInv,budget:rBudget,cashflow:rCashflow,match:rMatchPage,subscriptions:rSubscriptions,reports:rReport,backup:rBackup,cfg:rCfg};
  if(rs[pg])rs[pg]();
}
...
async function clientLogout(){try{await fetch('/api/client/logout',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({})});}catch(e){}location.href='/';}
...
async function initAIPage(){
  try{
    ...
    if(d.has_ai){
      if(badge){badge.textContent='✅ AI مفعّل';badge.style.background='rgba(52,211,153,.3)';}
      if(line){line.textContent=d.source==='owner'?'يستخدم مفتاح المالك':'يستخدم مفتاحك الخاص';}
      if(warn)warn.style.display='none';
    } else {
      if(badge){badge.textContent='❌ AI غير مفعّل';badge.style.background='rgba(239,68,68,.3)';}
      if(line)line.textContent='تواصل مع مدير النظام';
      if(warn)warn.style.display='block';
      ['ai-mkt-btn','ai-season-btn','ai-data-btn'].forEach(id=>{const e=document.getElementById(id);if(e)e.disabled=true;});
    }
  } catch(e){}
}

async function aiUpdateMarket(){
  const btn=document.getElementById('ai-mkt-btn');
  const res=document.getElementById('ai-mkt-result');
  if(btn){btn.disabled=true;btn.textContent='⏳ جارٍ البحث...';}
  if(res){res.style.display='none';}
  const city=document.getElementById('ai-mkt-city').value;
  const type=document.getElementById('ai-mkt-type').value;
  try{
    const r=await api('/api/market/websearch',{city,type});
    if(btn){btn.disabled=false;btn.textContent='🔍 بحث ويب + AI';}
    if(r.ok&&r.result){
      const d=r.result;
      if(res){
        res.style.display='block';
        res.innerHTML=`<div style="font-weight:700;margin-bottom:8px;color:var(--gd)">✅ تم تحديث أسعار السوق</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
            <div style="background:#fff;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--muted)">متوسط الفنادق</div><div style="font-size:16px;font-weight:700;color:var(--gd)">${d.hotel_avg||0} <span style="font-size:10px">ر.س</span></div></div>
            <div style="background:#fff;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--muted)">متوسط الشقق</div><div style="font-size:16px;font-weight:700;color:var(--gd)">${d.apart_avg||0} <span style="font-size:10px">ر.س</span></div></div>
            <div style="background:#fff;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--muted)">إشغال الفنادق</div><div style="font-size:16px;font-weight:700;color:var(--gd)">${d.hotel_occ||0}%</div></div>
            <div style="background:#fff;border-radius:8px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--muted)">إشغال الشقق</div><div style="font-size:16px;font-weight:700;color:var(--gd)">${d.apart_occ||0}%</div></div>
          </div>
          ${d.season?`<div style="margin-bottom:4px">🌙 <b>الموسم:</b> ${d.season} (×${d.season_factor||1})</div>`:''}
          ${d.recommendation?`<div style="margin-bottom:4px">💡 <b>التوصية:</b> ${d.recommendation}</div>`:''}
          ${d.insight?`<div style="color:#065f46">${d.insight}</div>`:''}
          <div style="margin-top:6px;font-size:9px;color:var(--muted)">المصدر: ${d.source||'AI'}</div>`;
      }
      // Update S state
      if(S.market_rates)S.market_rates.last_updated=new Date().toISOString();
      toast_('تم تحديث أسعار السوق ✓');
    } else {
      if(res){res.style.display='block';res.style.background='var(--rl)';res.innerHTML='❌ '+(r.error||'فشل التحليل');}
      toast_(r.error||'فشل التحليل',false);
    }
  } catch(e){
    if(btn){btn.disabled=false;btn.textContent='🔍 بحث ويب + AI';}
    toast_('خطأ في الاتصال',false);
  }
}

async function aiAnalyzeSeason(){
  const btn=document.getElementById('ai-season-btn');
  const res=document.getElementById('ai-season-result');
  if(btn){btn.disabled=true;btn.textContent='⏳ جارٍ التحليل...';}
  if(res)res.style.display='none';
  const city=document.getElementById('ai-season-city').value;
  try{
    const r=await api('/api/market/ai',{city,type:S.settings.type||'hotel'});
    if(btn){btn.disabled=false;btn.textContent='🌙 تحليل الموسم';}
    if(r.ok&&r.result){
      const d=r.result;
      if(res){
        res.style.display='block';
        res.style.background='var(--al)';
        res.innerHTML=`<div style="font-weight:700;margin-bottom:8px;color:var(--ad)">🌙 تحليل الموسم السياحي</div>
          <div style="margin-bottom:6px">📍 <b>الموسم الحالي:</b> ${d.season||'—'}</div>
          <div style="margin-bottom:6px">📈 <b>معامل التسعير:</b> ×${d.season_factor||1.0} ${d.season_factor>1?'(موسم مرتفع)':d.season_factor<1?'(موسم منخفض)':'(موسم عادي)'}</div>
          ${d.revpar?`<div style="margin-bottom:6px">💰 <b>RevPAR المتوقع:</b> ${d.revpar} ر.س</div>`:''}
          ${d.recommendation?`<div style="margin-bottom:6px">💡 <b>التوصية:</b> ${d.recommendation}</div>`:''}
          ${d.insight?`<div style="color:var(--ad)">${d.insight}</div>`:''}`;
      }
      toast_('تم تحليل الموسم ✓');
    } else {
      toast_(r.error||'فشل التحليل',false);
    }
  } catch(e){
...
async function aiAnalyzeData(){
  const btn=document.getElementById('ai-data-btn');
  const res=document.getElementById('ai-data-result');
  if(btn){btn.disabled=true;btn.textContent='⏳ جارٍ التحليل...';}
  if(res)res.style.display='none';
  // Build data summary
  const guests=S.guests||[];
  const active=guests.filter(g=>g.status==='active');
  const totalRev=guests.reduce((a,g)=>a+g.total,0);
  const rooms=S.settings.rooms||50;
  const occ=rooms?Math.round(active.length/rooms*100):0;
  const city=S.settings.city||'riyadh';
  const type=S.settings.type||'hotel';
  const mr=(S.market_rates&&S.market_rates.rates&&S.market_rates.rates[city])||{};
  const dataPayload={
    hotel_name:S.settings.name||'الفندق',
    city,type,rooms,
    active_guests:active.length,
    occupancy:occ,
    total_revenue:Math.round(totalRev),
    market_hotel_avg:mr.hotel_avg||0,
    market_apart_avg:mr.apart_avg||0,
    market_occ:mr.hotel_occ||mr.apart_occ||0,
    journal_count:(S.journal_entries||[]).length,
    invoices_count:(S.invoices||[]).length,
  };
  try{
    const r=await api('/api/ai/analyze-data',dataPayload);
    if(btn){btn.disabled=false;btn.textContent='📊 تحليل بياناتي الآن';}
    if(r.ok&&r.analysis){
      if(res){res.style.display='block';res.style.background='var(--gl)';res.innerHTML=r.analysis.replace(/
/g,'<br>');}
      toast_('تم التحليل ✓');
    } else {
      toast_(r.error||'فشل التحليل',false);
...
async function rSubscriptions(){
  ...
  const curName=planNames[subState.plan]||subState.plan||'—';
  ss('sub-plan-name',curName);
  ss('sub-exp',subState.expires||'—');
  // Load plans from admin
  try{
    ...
    // Current plan amount
    const curPlan=plans.find(p=>p.id===cur)||{price:0};
    ss('sub-next-amt',(curPlan.price||0)+' ر.س / شهر');
    // Render plans grid
    const grid=document.getElementById('plans-dynamic-grid');
    if(grid&&plans.length){
      grid.innerHTML=plans.map(p=>{
        const isCur=p.id===cur;
        const feats=(p.features||[]).map(f=>`<li style="padding:3px 0;border-bottom:1px solid rgba(0,0,0,.05);font-size:11px">✓ ${f}</li>`).join('');
        const excl=(p.excluded||[]).map(f=>`<li style="padding:3px 0;border-bottom:1px solid rgba(0,0,0,.05);font-size:11px;color:var(--muted);text-decoration:line-through">✕ ${f}</li>`).join('');
        return`<div style="border-radius:14px;border:2px solid ${isCur?p.color||'#3B6D11':p.color||'#E2E8F0'};padding:16px;background:${isCur?'#F0FFF4':'#fff'};position:relative;cursor:pointer;transition:.2s" onclick="selectPlanDyn('${p.id}','${p.name}',${p.price},'${p.color||'#185FA5'}')">
          ${p.badge?`<div style="position:absolute;top:-10px;right:50%;transform:translateX(50%);background:${p.color||'#185FA5'};color:#fff;font-size:10px;font-weight:700;padding:3px 12px;border-radius:20px">${p.badge}</div>`:''}
          ${isCur?'<div style="position:absolute;top:10px;left:10px;background:#3B6D11;color:#fff;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px">✓ خطتك الحالية</div>':''}
          <div style="font-size:15px;font-weight:700;color:${p.color||'#185FA5'};margin-bottom:4px;margin-top:${p.badge?'8':'0'}px">${p.name}</div>
          <div style="font-size:24px;font-weight:800;color:var(--text);margin-bottom:2px">${p.price}<span style="font-size:12px;font-weight:500;color:var(--muted)"> ر.س / ${p.period||'شهر'}</span></div>
          ${p.pos_devices?`<div style="font-size:10px;color:var(--muted);margin-bottom:8px">🖥️ ${p.pos_devices===0?'غير محدود':p.pos_devices} جهاز POS${p.max_guests?` · 👥 حتى ${p.max_guests} نزيل`:' · 👥 نزلاء غير محدود'}</div>`:''}
          <ul style="list-style:none;padding:0;margin:0 0 12px 0">${feats}${excl}</ul>
          <button class="btn sm" style="width:100%;background:${isCur?'#3B6D11':p.color||'#185FA5'};color:#fff;border-color:${isCur?'#3B6D11':p.color||'#185FA5'}" onclick="event.stopPropagation();selectPlanDyn('${p.id}','${p.name}',${p.price},'${p.color||'#185FA5'}')">${isCur?'خطتك الحالية ✓':'اختيار هذه الخطة'}</button>
        </div>`;
      }).join('');
    } else if(grid){
      grid.innerHTML='<div style="text-align:center;padding:20px;color:var(--muted)">لا توجد باقات متاحة حالياً — تواصل مع المدير</div>';
    }
    // Populate plan selector in activation form
    ...
    // Show bank info
    const bankCard=document.getElementById('sub-bank-card');
    const bankInfo=document.getElementById('sub-bank-info');
    if(bank.iban&&bankCard&&bankInfo){
      bankCard.style.display='block';
      bankInfo.innerHTML=`
        <div>اسم الحساب: <b>${bank.name||'—'}</b></div>
        <div>البنك: <b>${bank.bank||'—'}</b></div>
        <div style="font-family:monospace;font-size:13px;font-weight:700;color:var(--gd);margin-top:4px">IBAN: ${bank.iban}</div>
        ${bank.whatsapp?`<div style="margin-top:6px"><a href="https://wa.me/${bank.whatsapp}" target="_blank" style="color:var(--g);font-weight:600">📱 واتساب: ${bank.whatsapp}</a></div>`:''}
      `;
    }
  } catch(e){ console.error('rSubscriptions error',e); }
}
function selectPlanDyn(id,name,price,color){
  if(id===subState.plan){toast_('أنت على هذه الخطة بالفعل');return;}
  toast_(`لتفعيل باقة ${name} (${price} ر.س/شهر) — أدخل كود التفعيل أدناه أو حوّل للحساب البنكي`);
  const sel=document.getElementById('sub-plan-sel');
...
function activateSub(){
  ...
  const planName=planSel?planSel.options[planSel.selectedIndex]?.text:plan;
  ...
  api('/api/client/activate',{key:code}).then(r=>{
    if(r.ok){
      ...
      rSubscriptions();
      toast_(`تم تفعيل ${planName} حتى ${subState.expires} ✓`);
    } else {
      toast_(r.error||'كود التفعيل غير صحيح',false);
    }

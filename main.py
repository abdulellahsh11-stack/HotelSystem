    ...
    if(r.ok){
      ...
      if(bk.iban&&bc&&bi){
        ...
        bi.innerHTML=`
          ...
          <div style="font-family:monospace;font-size:14px;font-weight:700;color:var(--gd);padding:6px 0;letter-spacing:1px">
            IBAN: ${bk.iban}
          </div>
          ${bk.whatsapp?`<div style="margin-top:6px"><a href="https://wa.me/${bk.whatsapp}" target="_blank" style="color:var(--g);font-weight:600;font-size:13px">📱 تواصل عبر واتساب: ${bk.whatsapp}</a></div>`:''}
          <div style="margin-top:10px;padding:8px 10px;background:var(--al);border-radius:8px;font-size:11px;color:var(--ad);line-height:1.7">
            ⚠️ بعد التحويل، أرسل صورة الإيصال عبر واتساب أو افتح تذكرة دعم وسيتم إرسال كود التفعيل خلال 24 ساعة.
          </div>`;
        bc.scrollIntoView({behavior:'smooth',block:'center'});
      }
      toast_(`✅ تم إرسال طلب باقة ${name} — راجع بيانات التحويل أدناه`);
    } else {

...
def _hash_pass(pw: str) -> str:
    ...
    _SALT = os.environ.get("PASS_SALT", "HotelSaaS2025")
    return hashlib.pbkdf2_hmac("sha256", pw.encode(), _SALT.encode(), 260000).hex()

def _verify_pass(entered: str) -> bool:
    """مقارنة آمنة — تمنع Timing Attack"""
    return secrets.compare_digest(_hash_pass(entered), _ADMIN_HASH)
...
PORT        = 5051
# كلمة المرور مُشفّرة — لا تُخزَّن نصاً صريحاً أبداً
# القيمة الافتراضية = hash لـ Admin@2025#Hotel
_ADMIN_HASH = os.environ.get(
    "ADMIN_PASS_HASH",
    "a2b3683a64ba220757e826c1e1efd1943fec66d6d9763e7ff37c408437fd5d34"
)
...
class AdminStore:
    ...
    def _defaults(self):
        return {
            ...
            "admin_settings": {
                ...
                "plans": [
                    {
                        "id":"nano","name":"نانو","price":50,"period":"شهري",
                        "color":"#64748B","badge":"","branch_fee":0,"branch_enabled":False,
                        "pos_devices":1,"max_guests":30,
                        "features":[
                            "حتى 30 نزيل شهرياً",
                            "جهاز نقاط دفع واحد",
                            "إدارة الغرف والنزلاء",
                            "تقارير أساسية يومية",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                            "نسخ احتياطي يدوي",
                            "دعم فني عبر البريد الإلكتروني"
                        ],
                        "excluded":["مقارنة أسعار السوق","تحليل ذكاء اصطناعي","فروع متعددة","نسخ احتياطي تلقائي","تقارير متقدمة","API","دعم أولوية"]
                    },
                    {
                        "id":"micro","name":"مايكرو","price":99,"period":"شهري",
                        "color":"#0891B2","badge":"","branch_fee":0,"branch_enabled":False,
                        "pos_devices":1,"max_guests":60,
                        "features":[
                            "حتى 60 نزيل شهرياً",
                            "جهاز نقاط دفع واحد",
                            "إدارة الغرف والنزلاء والخدمات",
                        ...
                        "excluded":["مقارنة أسعار السوق","تحليل ذكاء اصطناعي","فروع متعددة","نسخ احتياطي تلقائي","API","دعم أولوية"]
                    },
                    {
                        "id":"starter","name":"المبتدئ","price":199,"period":"شهري",
                        "color":"#0F6E56","badge":"","branch_fee":0,"branch_enabled":False,
                        "pos_devices":2,"max_guests":150,
                        "features":[
                            "حتى 150 نزيل شهرياً",
                            "حتى 2 جهاز نقاط دفع",
                            "إدارة شاملة للغرف والنزلاء والخدمات",
                            "دفتر اليومية والمحاسبة الأساسية",
                            "مقارنة أسعار السوق (بيانات مرجعية)",
                            "تقارير يومية وشهرية",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                            "نسخ احتياطي تلقائي أسبوعي",
                            "دعم فني عبر التذاكر"
                        ],
                        "excluded":["تحليل ذكاء اصطناعي","فروع متعددة","نسخ احتياطي يومي","API","دعم أولوية"]
                    },
                    {
                        "id":"basic","name":"الأساسية","price":349,"period":"شهري",
                        "color":"#185FA5","badge":"","branch_fee":0,"branch_enabled":False,
                        "pos_devices":3,"max_guests":0,
                        "features":[
                            "نزلاء غير محدود",
                            "حتى 3 أجهزة نقاط دفع",
                            "إدارة شاملة مع فواتير الموردين",
                            "مقارنة أسعار السوق اليومية",
                            "تحليل ذكاء اصطناعي أساسي",
                            "ميزانية هرمية",
                            "تقارير PDF شهرية",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                        ...
                        "excluded":["فروع متعددة","API مفتوح","مدير حساب مخصص"]
                    },
                    {
                        "id":"pro","name":"الاحترافية","price":599,"period":"شهري",
                        "color":"#7C3AED","badge":"الأكثر طلباً","branch_fee":0,"branch_enabled":False,
                        "pos_devices":5,"max_guests":0,
                        "features":[
                            "نزلاء غير محدود",
                            "حتى 5 أجهزة نقاط دفع",
                            "إدارة شاملة مع كامل المحاسبة",
                            "مقارنة أسعار السوق الحية + ذكاء اصطناعي متقدم",
                            "تحليل موسمي وتوصيات تسعير",
                            "ميزانية هرمية وتدفق نقدي",
                            "تقارير PDF وExcel شهرية",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                            "نسخ احتياطي تلقائي يومي",
                            "دعم فني 24/7 بأولوية عالية"
                        ],
                        "excluded":["فروع متعددة","API مفتوح"]
                    },
                    {
                        "id":"advanced","name":"المتقدمة","price":899,"period":"شهري",
                        "color":"#B45309","badge":"","branch_fee":0,"branch_enabled":False,
                        "pos_devices":8,"max_guests":0,
                        "features":[
                            "نزلاء غير محدود",
                            "حتى 8 أجهزة نقاط دفع",
                            "كامل مميزات الاحترافية",
                            "تكامل نظام Opera/Cloudbeds",
                            "تقارير BI متقدمة",
                            "مقارنة منافسين متعددين",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                            "نسخ احتياطي فوري",
                            "API أساسي للتكامل",
                            "دعم فني مخصص"
                        ],
                        "excluded":["فروع متعددة","مدير حساب مخصص"]
                    },
                    {
                        "id":"premium","name":"المميزة","price":1299,"period":"شهري",
                        "color":"#DC2626","badge":"للفنادق الكبيرة","branch_fee":0,"branch_enabled":False,
                        "pos_devices":0,"max_guests":0,
                        "features":[
                            "نزلاء غير محدود",
                            "أجهزة نقاط دفع غير محدودة",
                            "كامل مميزات المتقدمة",
                            "فرع واحد مدمج",
                            "لوحة تحليل متقدمة",
                            "تقارير موحدة",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                    ...
                    {
                        "id":"enterprise","name":"المؤسسية","price":1999,"period":"شهري",
                        "color":"#1A56DB","badge":"للسلاسل","branch_fee":200,"branch_enabled":True,
                        "pos_devices":0,"max_guests":0,
                        "features":[
                            "نزلاء غير محدود",
                            "أجهزة نقاط دفع غير محدودة",
                            "إدارة مركزية لجميع الفروع",
                            "رسوم إضافية 200 ر.س لكل فرع مرتبط",
                            "تقارير موحدة للسلسلة الفندقية",
                            "ذكاء اصطناعي لكامل السلسلة",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                            "نسخ احتياطي فوري لحظي",
                            "API مفتوح كامل",
                            "مدير حساب مخصص",
                    ...
                    {
                        "id":"ultimate","name":"الشاملة","price":2999,"period":"شهري",
                        "color":"#0A3D2E","badge":"⭐ VIP","branch_fee":0,"branch_enabled":True,
                        "pos_devices":0,"max_guests":0,
                        "features":[
                            "نزلاء غير محدود",
                            "أجهزة نقاط دفع غير محدودة",
                            "فروع غير محدودة بدون رسوم إضافية",
                            "كامل جميع مميزات المنصة",
                            "تخصيص كامل حسب الطلب",
                            "تكامل GDS (Amadeus/Sabre/Travelport)",
                            "فواتير بضريبة القيمة المضافة (15%) وضريبة السياحة (2.5%)",
                            "نسخ احتياطي فوري لحظي مزدوج",
                            "API مفتوح + Webhooks",
                            "فريق دعم مخصص",
                            "تدريب وإعداد مجاني",
                            "SLA مضمون 99.9%"
    ...
    def save(self):
        ...
        # ── حفظ تلقائي في GitHub عند كل تغيير ──
        self._github_backup()

    def _github_backup(self):
        """يرفع admin_store.json إلى hotel-data-backup تلقائياً"""
        try:
            import base64 as _b64, urllib.request as _ur
            s      = self._data.get("admin_settings", {})
            token  = s.get("github_backup_token","") or os.environ.get("GITHUB_BACKUP_TOKEN","")
            repo   = s.get("github_backup_repo","")  or os.environ.get("GITHUB_BACKUP_REPO","")
            if not token or not repo:
                return  # لم يُعَدّ بعد
            # محتوى الملف
            with open(self._file, "rb") as f:
                content = _b64.b64encode(f.read()).decode()
            api_url = f"https://api.github.com/repos/{repo}/contents/data/admin_store.json"
            headers = {
                "Authorization": f"token {token}",
                "Content-Type":  "application/json",
                "Accept":        "application/vnd.github.v3+json",
                "User-Agent":    "dheuof-hotel-backup",
            }
            # احصل على SHA الحالي
            sha = None
            try:
                req = _ur.Request(api_url, headers=headers)
                res = _ur.urlopen(req, timeout=8)
                sha = json.loads(res.read()).get("sha","")
            except: pass
            # ارفع
            import json as _js
            payload = {
                "message": f"backup: admin_store — {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                "content": content,
            }
            if sha: payload["sha"] = sha
            req = _ur.Request(api_url, data=_js.dumps(payload).encode(), headers=headers, method="PUT")
            _ur.urlopen(req, timeout=15)
        except Exception as e:
            logging.debug(f"GitHub backup skip: {e}")
...
class AdminHandler(BaseHTTPRequestHandler):
    ...
    def _authed(self):
        ...
        for part in cookie.split(";"):
            ...
            if part.startswith("admin_token="):
                ...
                if exp and (exp if isinstance(exp, datetime) else datetime.fromisoformat(exp)) > datetime.now():
                    return True
        return False
    ...
    def do_GET(self):
        ...
        routes = {
            ...
            "/api/admin/store/export":  self._p_admin_store_export,
        }
        fn = routes.get(path)
        if fn: fn(p)
    ...
    def do_POST(self):
        ...
        routes = {
            ...
            "/api/admin/update/status":       self._p_update_status,
            "/api/admin/update/github-push":  self._p_github_push,
            "/api/admin/store/export":        self._p_admin_store_export,
    ...
    def _p_logout(self, b):
        ...
        # حفظ sessions على الديسك
        try:
            _sf = os.path.join(os.path.expanduser("~"),"HotelAdmin","data","sessions.json")
            _clean = {k:v.isoformat() if hasattr(v,'isoformat') else str(v) for k,v in sessions.items()}
            with open(_sf,'w',encoding='utf-8') as _sf2: json.dump(_clean,_sf2)
    ...
    def _p_client_add(self, b):
        ...
        client = {
            ...
            "last_login":  None,
            # ── بيانات التكامل والإعدادات ──
            "mk":          b.get("mk", ""),          # Merchant Key — Geidea
            "api_key":     b.get("api_key", ""),     # API Key العميل
            "pms_type":    b.get("pms_type", ""),    # نوع PMS (opera/cloudbeds/...)
            "pms_url":     b.get("pms_url", ""),     # رابط PMS
            "pms_key":     b.get("pms_key", ""),     # مفتاح PMS
            # ── إعدادات المنشأة الكاملة ──
            "facility_settings": {
                "name":        b.get("hotel_name", name),
                "type":        b.get("hotel_type", "hotel"),
                "city":        b.get("city", ""),
                "region":      b.get("region", ""),
                "phone":       b.get("phone", ""),
                "email":       b.get("email", ""),
                "rooms":       int(b.get("rooms", 0)),
                "units_count": int(b.get("units_count", 0)),
            },
            "custom_settings": {},
    ...
    def _p_client_update(self, b):
        ...
        for c in clients:
            if str(c.get("id")) == str(cid):
                for k in ["name","hotel_name","email","phone","city","hotel_type","rooms","plan","notes",
                          "bank_account","mk","api_key","pms_type","pms_url","pms_key"]:
                    if k in b: c[k] = b[k]
                # تحديث facility_settings تلقائياً
                fs = c.get("facility_settings", {})
                for k in ["hotel_name","hotel_type","city","region","phone","email","vat_number","rooms","units_count"]:
                    if k in b:
                        mapped = {"hotel_name":"name","hotel_type":"type"}.get(k, k)
                        fs[mapped] = b[k]
                c["facility_settings"] = fs
    ...
    def _p_client_delete(self, b):
        cid = str(b.get("id"))
        clients = [c for c in adm.get("clients", []) if str(c.get("id")) != cid]
        adm.set("clients", clients)
        # حفظ IDs المحذوفة لمنع عودتها من GitHub
        deleted = adm.get("deleted_client_ids", [])
        if cid not in deleted:
            deleted.append(cid)
        adm.set("deleted_client_ids", deleted)
        self._json({"ok": True})
    ...
    def _p_settings_save(self, b):
        ...
        for k in ["owner_name","owner_email","owner_phone","owner_whatsapp","owner_account_name","contact_message","system_name","bank_iban","bank_name","owner_iban","owner_bank","trial_days","prices","plans","ai_market_competitors","github_token","github_repo","github_branch","github_backup_token","github_backup_repo"]:
            if k in b: s[k] = b[k]
...
{{
  ...
  "source": "web_search+claude"
    ...
    def _p_github_push(self, b):
        """رفع ملفات مباشرة لـ GitHub عبر API"""
        import base64 as _b64, urllib.request as _ur, urllib.error as _ue, json as _js, os as _os, ast as _ast
        s      = adm.get("admin_settings", {})
        token  = s.get("github_token","") or _os.environ.get("GITHUB_TOKEN","")
        repo   = s.get("github_repo","")  or _os.environ.get("GITHUB_REPO","abdulellahsh11-stack/HotelSystem")
        branch = s.get("github_branch","") or _os.environ.get("GITHUB_BRANCH","main")
        if not token:
            self._json({"ok":False,"error":"GITHUB_TOKEN غير موجود في Railway Variables"}); return

        files_data = b.get("files", [])  # [{name, content_b64}]
        desc = b.get("description","تحديث dheuof")
        if not files_data:
            self._json({"ok":False,"error":"لا توجد ملفات للرفع"}); return

        allowed = {"main.py","main_admin.py","unified_server.py"}
        results = []

        for f in files_data:
            fname   = _os.path.basename(f.get("name",""))
            content = f.get("content_b64","")
            if fname not in allowed:
                results.append({"file":fname,"ok":False,"error":"ملف غير مدعوم"}); continue
            # فحص syntax
            try:
                _ast.parse(_b64.b64decode(content).decode("utf-8"))
            except SyntaxError as e:
                results.append({"file":fname,"ok":False,"error":f"خطأ syntax السطر {e.lineno}: {e.msg}"}); continue
            # احصل على SHA
            ...
            headers = {
                ...
                "Accept": "application/vnd.github.v3+json",
                "Content-Type": "application/json",
                "User-Agent": "dheuof-admin",
            }
            sha = None
            try:
                req = _ur.Request(api_url, headers=headers)
                with _ur.urlopen(req, timeout=10) as r:
                    sha = _js.loads(r.read()).get("sha","")
            except: pass
            # ارفع
            payload = {"message":f"{desc} — {fname}","content":content,"branch":branch}
            if sha: payload["sha"] = sha
            try:
                req = _ur.Request(api_url, data=_js.dumps(payload).encode(), headers=headers, method="PUT")
                with _ur.urlopen(req, timeout=20) as r:
                    code = r.status
                results.append({"file":fname,"ok":True,"code":code})
                # سجّل
                updates = adm.get("update_history",[])
                updates.append({"file":fname,"description":desc,"uploaded_at":_iso(),"source":"github_push"})
                adm.set("update_history", updates[-20:])
                logging.info(f"GitHub push: {fname} → {repo}")
            except _ue.HTTPError as e:
                err = e.read().decode()
                results.append({"file":fname,"ok":False,"error":f"HTTP {e.code}: {err[:100]}"})
            except Exception as e:
                results.append({"file":fname,"ok":False,"error":str(e)})

        ok_count = sum(1 for r in results if r.get("ok"))
        self._json({"ok": ok_count>0, "results": results,
                    "message": f"✅ رُفع {ok_count}/{len(results)} ملفات — Railway يعيد النشر الآن" if ok_count else "❌ فشل الرفع"})

    def _p_admin_store_export(self, b=None):
        """تصدير admin_store.json كاملاً للتنزيل"""
        import json as _js, copy as _cp
        data = _cp.deepcopy(adm._data)
        # إخفاء التوكنات الحساسة
        s = data.get("admin_settings", {})
        for k in ["github_token","github_backup_token"]:
            if k in s: s[k] = "***"
        self._json({"ok": True, "data": data, "exported_at": _iso()})
...
<div class="sb">
  ...
  <div class="ni" onclick="go('plans')"><span class="ni-ic">💎</span>إدارة الباقات والأسعار</div>
  ...
  <div class="content">
      ...
      <div class="card cb">
        <div class="ch">
          ...
          <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
            <input id="cl-search" placeholder="🔍 بحث..." style="border:0.5px solid var(--border);border-radius:8px;padding:5px 10px;font-size:11px;background:var(--bg);color:var(--text);width:130px" oninput="filterClients()"/>
            ...
            <button class="btn sm" id="btn-f-new" onclick="setSmartFilter('new')" style="font-size:10px;background:#854F0B;color:#fff;border-color:#854F0B">🆕 سجّل ولم يتصل</button>
            <button class="btn sm" id="btn-f-soon" onclick="setSmartFilter('soon')" style="font-size:10px;background:#A32D2D;color:#fff;border-color:#A32D2D">⏰ تنتهي قريباً</button>
            <button class="btn sm" id="btn-f-all" onclick="setSmartFilter('all')" style="font-size:10px">↩ الكل</button>
            <button class="btn bp sm" onclick="go('register')">+ عميل جديد</button>
          </div>
          <div id="smart-filter-bar" style="display:none;margin-top:8px;padding:8px 12px;background:var(--pl);border-radius:8px;font-size:11px;color:var(--p);font-weight:700"></div>
        </div>
        <div style="overflow-x:auto"><table class="tbl">
          <thead><tr><th>العميل</th><th>التواصل</th><th>المدينة</th><th>الخطة</th><th>الحالة</th><th>أيام الاستخدام</th><th>آخر دخول</th><th>انتهاء التجربة</th><th>الإيراد</th><th>إجراءات</th></tr></thead>
    ...
    <div class="pg" id="pg-register">
      <div class="card cb">
        ...
        <div style="font-size:10px;color:var(--muted);padding:6px 9px;background:var(--bg);border-radius:8px;margin-bottom:10px">البيانات الأساسية (★) تُحفظ ثابتة مع العميل — تُستخدم في الفواتير والتقارير</div>
        <div class="g2">
          ...
          <div class="fg"><label>المنطقة ★</label><input id="rg-region" placeholder="المنطقة الشرقية..."/></div>
          ...
          <div class="fg"><label>عدد الغرف / الوحدات ★</label><input type="number" id="rg-rooms" placeholder="50" min="1"/></div>
        ...
        <div style="font-size:11px;font-weight:700;color:var(--muted);margin:8px 0 6px">⚙️ إعدادات التكامل (اختيارية)</div>
        <div class="g2">
          <div class="fg"><label>Merchant Key (MK) — Geidea</label><input id="rg-mk" type="password" placeholder="MK-xxxxxxxx"/></div>
          <div class="fg"><label>API Key للعميل</label><input id="rg-api-key" placeholder="api_xxxxxxxx"/></div>
          <div class="fg"><label>نوع PMS</label><select id="rg-pms-type"><option value="">لا يوجد</option><option value="opera">Oracle Opera</option><option value="cloudbeds">Cloudbeds</option><option value="mews">Mews</option><option value="custom">Custom API</option></select></div>
          <div class="fg"><label>رابط PMS</label><input id="rg-pms-url" placeholder="https://pms.hotel.com/api"/></div>
          <div class="fg"><label>مفتاح PMS</label><input id="rg-pms-key" type="password" placeholder="sk-xxxxxxxx"/></div>
        </div>
    ...
    <div class="pg" id="pg-plans">
      ...
      <!-- GitHub — التحديث -->
      <div class="card" style="border-top:3px solid #24292F">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:#24292F;color:#fff">🔄</div>
        <div><div style="font-size:13px;font-weight:700">GitHub — رفع التحديثات</div>
        <div style="font-size:11px;color:var(--muted)">الملف يذهب لـ GitHub ← Railway يُحدَّث تلقائياً</div></div></div></div>
        <div class="g2" style="margin-bottom:8px">
          <div class="fg"><label>GitHub Token (للتحديث)</label><input id="cfg-github-token" type="password" placeholder="ghp_xxxxxxxxxxxx" style="direction:ltr;font-family:monospace"/><div style="font-size:10px;color:var(--muted);margin-top:3px">scope: repo</div></div>
          <div class="fg"><label>مستودع البرنامج (owner/repo)</label><input id="cfg-github-repo" placeholder="abdulellahsh11-stack/HotelSystem" style="direction:ltr;font-family:monospace"/></div>
        </div>
        <div class="fg" style="max-width:200px;margin-bottom:8px"><label>الفرع</label><input id="cfg-github-branch" placeholder="main" style="direction:ltr;font-family:monospace" value="main"/></div>
        <div style="display:flex;gap:8px"><button class="btn sm" style="background:#24292F;color:#fff;border-color:#24292F" onclick="saveGitHubSettings()">💾 حفظ</button><button class="btn sm" onclick="testGitHubConnection()">🔍 اختبار</button></div>
        <div id="github-test-result" style="margin-top:8px;font-size:11px;display:none"></div>
      </div>

      <!-- GitHub — نسخ احتياطي للبيانات -->
      <div class="card" style="border-top:3px solid #2DA44E">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:#2DA44E22;color:#2DA44E">💾</div>
        <div><div style="font-size:13px;font-weight:700">GitHub — نسخ احتياطي تلقائي للبيانات</div>
        <div style="font-size:11px;color:var(--muted)">كل تغيير في بيانات العملاء يُرفع لـ hotel-data-backup تلقائياً</div></div></div></div>
        <div class="g2" style="margin-bottom:8px">
          <div class="fg"><label>GitHub Token (للنسخ الاحتياطي)</label><input id="cfg-backup-token" type="password" placeholder="ghp_xxxxxxxxxxxx" style="direction:ltr;font-family:monospace"/></div>
          <div class="fg"><label>مستودع النسخ الاحتياطي</label><input id="cfg-backup-repo" placeholder="abdulellahsh11-stack/hotel-data-backup" style="direction:ltr;font-family:monospace"/></div>
        </div>
        <div style="display:flex;gap:8px"><button class="btn sm" style="background:#2DA44E;color:#fff;border-color:#2DA44E" onclick="saveBackupSettings()">💾 حفظ</button><button class="btn sm" onclick="testBackupNow()">🔄 نسخ الآن</button></div>
        <div id="backup-gh-result" style="margin-top:8px;font-size:11px;display:none"></div>
      </div>
    </div>
    ...
    <div class="pg" id="pg-update">
      ...
      <!-- ── خطوة 1: تنزيل البيانات ── -->
      <div class="card cg" style="margin-bottom:10px">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:var(--gl);color:var(--g)">💾</div>
          <div>
            <div style="font-size:13px;font-weight:700">الخطوة 1 — تنزيل بيانات العملاء</div>
            <div style="font-size:10px;color:var(--muted);margin-top:2px">يجب تنزيلها قبل الرفع — لن تفقد أي بيانات</div>
          </div>
        </div></div>
        <div style="background:var(--gl);border-radius:8px;padding:10px 12px;margin-bottom:10px;font-size:11px;color:var(--gd);font-weight:600">
          ✅ يُنزّل admin_store.json — يحتوي كل العملاء والاشتراكات والمفاتيح والمدفوعات
        </div>
        <button class="btn bg2" onclick="downloadAdminStore()" style="width:100%">⬇️ تنزيل بيانات العملاء الآن</button>
        <div id="backup-status" style="margin-top:8px;font-size:11px;color:var(--muted);text-align:center"></div>
      </div>

      <!-- ── خطوة 2: رفع لـ GitHub ── -->
      <div class="card" style="border-top:2px solid var(--u);margin-bottom:10px">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:var(--ul);color:var(--u)">🚀</div>
          <div>
            <div style="font-size:13px;font-weight:700">الخطوة 2 — رفع التحديث لـ GitHub</div>
            <div style="font-size:10px;color:var(--muted);margin-top:2px">Railway يسحب ويعيد النشر تلقائياً</div>
          </div>
        </div></div>
        <div style="background:var(--ul);border-radius:8px;padding:10px 12px;margin-bottom:12px;font-size:11px;color:var(--ud);">
          ⚡ يرفع: <b>main.py + main_admin.py + unified_server.py</b> مباشرة لـ GitHub<br/>
          ⚡ الـ Token محفوظ في Railway Variables — لا تحتاج إدخاله<br/>
          ⚡ بعد الرفع انتظر دقيقة ثم أعد تحميل الموقع
        </div>

        <!-- Upload zone للملفات -->
        <div style="font-size:11px;font-weight:700;color:var(--muted);margin-bottom:8px">اختر الملفات المحدّثة (1-3 ملفات):</div>
        <div id="upload-zone" style="border:2px dashed var(--border);border-radius:12px;padding:24px;text-align:center;cursor:pointer;transition:all .2s;margin-bottom:10px;"
          onclick="document.getElementById('file-input').click()"
          ondragover="event.preventDefault();this.style.borderColor='var(--u)';this.style.background='var(--ul)'"
          ...
          <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:3px">اسحب الملفات هنا أو اضغط للاختيار</div>
          <div style="font-size:10px;color:var(--muted)">main.py — main_admin.py — unified_server.py</div>
          <input type="file" id="file-input" accept=".py" multiple style="display:none" onchange="handleFileSelect(event)"/>
        </div>

        <!-- قائمة الملفات المختارة -->
        <div id="files-selected" style="display:none;margin-bottom:10px"></div>

        <!-- وصف -->
        <div class="fg" style="margin-bottom:10px">
          <label>وصف التحديث</label>
          <input id="update-desc" placeholder="مثال: حذف الحساب البنكي + تحسين لوحة التحكم"/>
        </div>

        <button class="btn bu2" id="github-push-btn" onclick="pushToGitHub()" style="width:100%;font-size:13px;padding:10px">
          🚀 رفع لـ GitHub الآن
        </button>
        <div id="push-progress" style="display:none;margin-top:10px"></div>
      </div>
    ...
    <div class="pg" id="pg-settings">
      ...
      <div class="card" style="border-top:3px solid #7C3AED;margin-top:12px">
        <div class="ch">
          <div class="ch-l"><div class="cico" style="background:#F5F3FF;color:#7C3AED">🤖</div>إعدادات Claude AI</div>
          <div id="ai-status-badge" style="font-size:10px;padding:4px 10px;border-radius:20px;background:#F5F3FF;color:#7C3AED;font-weight:700">—</div>
        </div>
        <div style="background:#F5F3FF;border-radius:10px;padding:12px;margin-bottom:12px;font-size:11px;color:#4C1D95;line-height:1.8">
          🔑 مفتاح Claude API الخاص بك يُستخدم لجميع ميزات الذكاء الاصطناعي.<br>
          يمكنك مشاركته مع العملاء <b>دون أن يرونه</b>.
        </div>
        <div class="fg">
          <label>مفتاح Claude API</label>
          <div style="display:flex;gap:6px">
            <input id="cfg-claude-key" type="password" placeholder="sk-ant-api03-..." style="flex:1;font-family:monospace;direction:ltr"/>
            <button class="btn sm" style="background:#7C3AED;color:#fff;border-color:#7C3AED" onclick="testClaudeKey()">اختبار</button>
          </div>
          <div id="cfg-key-test-result" style="font-size:10px;margin-top:4px;display:none"></div>
        </div>
        <div style="display:flex;flex-direction:column;gap:8px;margin:10px 0">
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border-radius:9px;border:1.5px solid var(--border)">
            <input type="checkbox" id="cfg-share-key" style="width:16px;height:16px;accent-color:#7C3AED"/>
            <div>
              <div style="font-size:12px;font-weight:700">مشاركة المفتاح مع العملاء</div>
              <div style="font-size:10px;color:var(--muted)">العملاء يستخدمون AI بمفتاحك دون رؤيته</div>
            </div>
          </label>
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:9px 12px;border-radius:9px;border:1.5px solid var(--border)">
            <input type="checkbox" id="cfg-ai-enabled" style="width:16px;height:16px;accent-color:#7C3AED" checked/>
            <div>
              <div style="font-size:12px;font-weight:700">تفعيل AI للعملاء</div>
              <div style="font-size:10px;color:var(--muted)">إيقافه يعطّل جميع ميزات AI للعملاء</div>
            </div>
          </label>
        </div>
        <button class="btn" style="background:#7C3AED;color:#fff;border-color:#7C3AED;width:100%" onclick="saveAISettings()">💾 حفظ إعدادات AI</button>
        <div id="ai-save-msg" style="font-size:11px;margin-top:6px;display:none"></div>
      </div>

      <div class="card" style="border-top:3px solid #238636;margin-top:12px">
        <div class="ch">
          <div class="ch-l"><div class="cico" style="background:#ECFDF5;color:#065F46">☁️</div>النسخ الاحتياطي السحابي</div>
          <div id="backup-badge" style="font-size:10px;padding:4px 10px;border-radius:20px;background:#F1F5F9;color:#475569;font-weight:700">—</div>
        </div>
        <div style="background:#ECFDF5;border-radius:10px;padding:12px;margin-bottom:12px;font-size:11px;color:#065F46;line-height:1.8">
          ✅ بيانات التعريف تُحفظ تلقائياً في GitHub عند كل تعديل.<br>
          🔄 تُسترجع تلقائياً عند كل إعادة تشغيل.
        </div>
        <div style="display:flex;gap:8px">
          <button class="btn bg2" onclick="checkBackupStatus()" style="flex:1">🔍 فحص الحالة</button>
          <button class="btn" onclick="restoreBackup()" style="flex:1;border-color:#238636;color:#238636">🔄 استرجاع</button>
        </div>
        <div id="backup-result" style="margin-top:8px;font-size:11px;display:none"></div>
      </div>
    </div>
...
async function callAI(cmd, autoExec=false){
  ...
  const key = gv('ai-key-input')||D.settings.claude_key||gv('cfg-claude-key')||'';
  try{
...
//  CLIENTS
// ═══════════════════════════════════════════════
let _smartFilter = 'all';

function setSmartFilter(f){
  _smartFilter = f;
  const bar = document.getElementById('smart-filter-bar');
  const labels = {
    new:  '🆕 يُظهر: سجّلوا بالتجربة ولم يدخلوا للنظام (days_active = 0)',
    soon: '⏰ يُظهر: تنتهي تجربتهم خلال 5 أيام',
    all:  ''
  };
  if(bar){ bar.textContent = labels[f] || ''; bar.style.display = f === 'all' ? 'none' : 'block'; }
  // تمييز الزر النشط
  ['new','soon','all'].forEach(k=>{
...
function filterClients(){
  const q  = gv('cl-search').toLowerCase();
  const st = gv('cl-filter');
  const cs = analytics_data.client_stats || {};
  let fl = D.clients.filter(c =>
    (!q  || (c.name+c.hotel_name+c.email).toLowerCase().includes(q)) &&
    (!st || c.status === st)
  );
  if(_smartFilter === 'new'){
    // سجّلوا بالتجربة ولم يدخلوا (days_active = 0 أو غير موجود)
    fl = fl.filter(c => {
      const stat = cs[String(c.id)] || {};
      return c.status === 'trial' && (stat.days_active || 0) === 0;
    });
  } else if(_smartFilter === 'soon'){
    // تنتهي تجربتهم خلال 5 أيام
    fl = fl.filter(c => {
      const dl = daysLeft(c.trial_end);
      return c.status === 'trial' && dl !== null && dl >= 0 && dl <= 5;
    });
    // ترتيب تصاعدي حسب الأيام المتبقية
    fl.sort((a,b) => daysLeft(a.trial_end) - daysLeft(b.trial_end));
  }
  renderClientsTable(fl);
}
function rClients(){ setSmartFilter(_smartFilter); }
function renderClientsTable(list){
  const prices=D.settings.prices||{pro:299,enterprise:799};
  const cs=analytics_data.client_stats||{};
  const el=document.getElementById('clients-body');if(!el)return;
  el.innerHTML=list.length?list.map(c=>{const dl=daysLeft(c.trial_end);const pr=prices[c.plan]||0;const stat=cs[String(c.id)]||{days_active:0};const lastLogin=stat.last_login?new Date(stat.last_login).toLocaleDateString('ar-SA',{month:'short',day:'numeric'}):'لم يدخل';const neverIn=!stat.last_login||(stat.days_active||0)===0;
return`<tr style="${neverIn&&c.status==='trial'?'background:rgba(133,79,11,.08)':''}"><td><div style="font-weight:700">${c.hotel_name||c.name}</div><div style="font-size:9px;color:var(--muted);font-weight:600">${c.name||'—'}</div>${neverIn&&c.status==='trial'?'<div style="font-size:9px;font-weight:700;color:#854F0B">⚠️ لم يدخل بعد</div>':''}</td><td style="font-size:10px;line-height:1.7">${c.phone?`<a href="https://wa.me/966${c.phone.replace(/^0/,'')}" target="_blank" style="color:var(--g);font-weight:700;text-decoration:none;display:block">📱 ${c.phone}</a>`:'<span style="color:var(--muted)">لا جوال</span>'}${c.email?`<a href="mailto:${c.email}" style="color:var(--p);font-size:9px;text-decoration:none;display:block">✉️ ${c.email}</a>`:'<span style="font-size:9px;color:var(--muted)">لا إيميل</span>'}</td><td style="font-size:10px">${c.city||'—'}</td><td>${planPill(c.plan)}</td><td>${stPill(c.status)}</td><td style="text-align:center;font-weight:700;color:var(--p)">${stat.days_active||0}</td><td style="font-size:10px;color:${neverIn?'var(--r)':'var(--muted)'};font-weight:${neverIn?'700':'400'}">${lastLogin}</td><td style="font-size:10px;color:${dl!==null&&dl<=3?'var(--r)':'var(--muted)'}">${c.trial_end||'—'}${dl!==null?' ('+dl+' يوم)':''}</td><td style="font-weight:700;color:var(--g)">${pr?FR(pr):'—'}</td><td style="white-space:nowrap"><button class="btn sm" onclick="showClientDetail(${c.id})">تفاصيل</button><button class="btn sm bg2" onclick="showExtend(${c.id})">تمديد</button><button class="btn sm bp" onclick="assignKey(${c.id})">مفتاح</button>${c.status==='suspended'?`<button class="btn sm bg2" onclick="toggleClient(${c.id},'activate')">تفعيل</button>`:`<button class="btn sm" style="color:var(--r)" onclick="toggleClient(${c.id},'suspend')">إيقاف</button>`}</td></tr>`;}).join(''):'<tr><td colspan="9" style="text-align:center;padding:14px;color:var(--muted)">لا يوجد عملاء</td></tr>';
}
function showClientDetail(id){
  ...
  document.getElementById('modal-client-body').innerHTML=`
    <div style="background:var(--bg);border-radius:10px;padding:12px;margin-bottom:10px;border:1px solid var(--border)">
      <div style="font-size:15px;font-weight:800;margin-bottom:2px">${c.hotel_name||c.name}</div>
      <div style="font-size:11px;color:var(--muted);margin-bottom:8px">👤 ${c.name||'—'} &nbsp;|&nbsp; 📍 ${c.city||'—'} &nbsp;|&nbsp; 🏨 ${c.hotel_type||'—'}</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ${c.phone?`<a href="https://wa.me/966${c.phone.replace(/^0/,'')}" target="_blank" style="display:inline-flex;align-items:center;gap:4px;padding:6px 12px;background:#25D366;color:#fff;border-radius:8px;text-decoration:none;font-size:11px;font-weight:700">📱 واتساب ${c.phone}</a>`:''}
        ${c.email?`<a href="mailto:${c.email}" style="display:inline-flex;align-items:center;gap:4px;padding:6px 12px;background:var(--pl);color:var(--pd);border-radius:8px;text-decoration:none;font-size:11px;font-weight:700">✉️ ${c.email}</a>`:''}
      </div>
    </div>
    ...
    <div class="g2">
      <div><div style="font-size:10px;color:var(--muted)">اسم صاحب المنشأة</div><div style="font-size:12px;margin-top:2px;font-weight:700">${c.name||'—'}</div></div>
      <div><div style="font-size:10px;color:var(--muted)">اسم المنشأة</div><div style="font-size:12px;margin-top:2px;font-weight:700">${c.hotel_name||'—'}</div></div>
      <div><div style="font-size:10px;color:var(--muted)">البريد الإلكتروني</div><div style="font-size:11px;margin-top:2px">${c.email||'—'}</div></div>
      <div><div style="font-size:10px;color:var(--muted)">رقم الجوال</div><div style="font-size:11px;margin-top:2px;font-weight:700">${c.phone||'—'}</div></div>
      <div><div style="font-size:10px;color:var(--muted)">المنطقة</div><div style="font-size:11px;margin-top:2px">${c.city||'—'}</div></div>
      <div><div style="font-size:10px;color:var(--muted)">نوع المنشأة</div><div style="font-size:11px;margin-top:2px">${c.hotel_type||'—'}</div></div>
      ...
      ${c.facility_settings?.region?`<div><div style="font-size:10px;color:var(--muted)">المنطقة</div><div style="font-size:11px;margin-top:2px">${c.facility_settings.region}</div></div>`:''}
    </div>
    ${c.mk||c.api_key||c.pms_type?`<div style="margin:10px 0;padding:10px 12px;background:#0F172A;border-radius:10px">
      <div style="font-size:10px;font-weight:700;color:#94A3B8;margin-bottom:8px">⚙️ بيانات التكامل</div>
      ${c.mk?`<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px"><span style="font-size:10px;color:#64748B;min-width:80px">MK (Geidea)</span><span style="font-family:monospace;font-size:11px;color:#10B981">${c.mk}</span></div>`:''}
      ${c.api_key?`<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px"><span style="font-size:10px;color:#64748B;min-width:80px">API Key</span><span style="font-family:monospace;font-size:11px;color:#60A5FA">${c.api_key}</span></div>`:''}
      ${c.pms_type?`<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px"><span style="font-size:10px;color:#64748B;min-width:80px">PMS</span><span style="font-size:11px;color:#F59E0B">${c.pms_type}${c.pms_url?' — '+c.pms_url:''}</span></div>`:''}
    </div>`:''}
    ...
    <div style="display:flex;gap:6px;margin-top:10px;flex-wrap:wrap">
      ...
      <button class="btn sm" style="background:#FFFBEB;color:#92400E;border-color:#D97706" onclick="unlockClientSettings(${c.id})" title="يتيح للعميل تعديل الاسم والمدينة والجوال وغيرها مرة واحدة">🔓 فتح الإعدادات</button>
...
async function unlockClientSettings(id){if(!confirm('هل تريد فتح إعدادات المنشأة؟ سيتمكن العميل من تعديلها مرة واحدة.'))return;const r=await api('/api/admin/clients/unlock-settings',{client_id:id});if(r.ok){toast_('✅ تم فتح إعدادات المنشأة — بإمكان العميل تعديلها الآن');}else toast_(r.error||'خطأ في الفتح',false);}
...
function rPlans(){
  ...
  // GitHub التحديث
  if(s.github_token){const el=document.getElementById('cfg-github-token');if(el)el.value=s.github_token;}
  if(s.github_repo){const el=document.getElementById('cfg-github-repo');if(el)el.value=s.github_repo;}
  if(s.github_branch){const el=document.getElementById('cfg-github-branch');if(el)el.value=s.github_branch;}
  // GitHub النسخ الاحتياطي
  if(s.github_backup_token){const el=document.getElementById('cfg-backup-token');if(el)el.value=s.github_backup_token;}
  if(s.github_backup_repo){const el=document.getElementById('cfg-backup-repo');if(el)el.value=s.github_backup_repo;}
  const cm=document.getElementById('cfg-contact-msg'); if(cm&&s.contact_message)cm.value=s.contact_message;
}

function getDefaultPlans(){
  return [
    {id:'nano',name:'نانو',price:50,period:'شهري',color:'#64748B',badge:'',branch_fee:0,branch_enabled:false,pos_devices:1,max_guests:30,
     features:['حتى 30 نزيل شهرياً','جهاز نقاط دفع واحد','إدارة الغرف والنزلاء','تقارير أساسية يومية','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي يدوي','دعم فني عبر البريد'],
     excluded:['مقارنة أسعار السوق','تحليل ذكاء اصطناعي','فروع متعددة','نسخ احتياطي تلقائي','API','دعم أولوية']},
    {id:'micro',name:'مايكرو',price:99,period:'شهري',color:'#0891B2',badge:'',branch_fee:0,branch_enabled:false,pos_devices:1,max_guests:60,
     features:['حتى 60 نزيل شهرياً','جهاز نقاط دفع واحد','إدارة الغرف والنزلاء والخدمات','دفتر اليومية المحاسبي','تقارير يومية وشهرية','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي يدوي','دعم فني عبر التذاكر'],
     excluded:['مقارنة أسعار السوق','تحليل ذكاء اصطناعي','فروع متعددة','نسخ احتياطي تلقائي','API','دعم أولوية']},
    {id:'starter',name:'المبتدئ',price:199,period:'شهري',color:'#0F6E56',badge:'',branch_fee:0,branch_enabled:false,pos_devices:2,max_guests:150,
     features:['حتى 150 نزيل شهرياً','حتى 2 جهاز نقاط دفع','إدارة شاملة للغرف والخدمات','دفتر اليومية والمحاسبة','مقارنة أسعار السوق (مرجعية)','تقارير يومية وشهرية','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي أسبوعي','دعم فني عبر التذاكر'],
     excluded:['تحليل ذكاء اصطناعي','فروع متعددة','نسخ احتياطي يومي','API','دعم أولوية']},
    {id:'basic',name:'الأساسية',price:349,period:'شهري',color:'#185FA5',badge:'',branch_fee:0,branch_enabled:false,pos_devices:3,max_guests:0,
     features:['نزلاء غير محدود','حتى 3 أجهزة نقاط دفع','إدارة شاملة مع فواتير الموردين','مقارنة أسعار السوق اليومية','تحليل ذكاء اصطناعي أساسي','ميزانية هرمية','تقارير PDF شهرية','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي يومي','دعم فني بأولوية'],
     excluded:['فروع متعددة','API مفتوح','مدير حساب مخصص']},
    {id:'pro',name:'الاحترافية',price:599,period:'شهري',color:'#7C3AED',badge:'الأكثر طلباً',branch_fee:0,branch_enabled:false,pos_devices:5,max_guests:0,
     features:['نزلاء غير محدود','حتى 5 أجهزة نقاط دفع','كامل المحاسبة والتقارير','مقارنة أسعار السوق الحية + ذكاء اصطناعي متقدم','تحليل موسمي وتوصيات تسعير','ميزانية هرمية وتدفق نقدي','تقارير PDF وExcel شهرية','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي يومي','دعم فني 24/7 أولوية عالية'],
     excluded:['فروع متعددة','API مفتوح']},
    {id:'advanced',name:'المتقدمة',price:899,period:'شهري',color:'#B45309',badge:'',branch_fee:0,branch_enabled:false,pos_devices:8,max_guests:0,
     features:['نزلاء غير محدود','حتى 8 أجهزة نقاط دفع','كامل مميزات الاحترافية','تكامل Opera/Cloudbeds','تقارير BI متقدمة','مقارنة منافسين متعددين','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي فوري','API أساسي','دعم فني مخصص'],
     excluded:['فروع متعددة','مدير حساب مخصص']},
    {id:'premium',name:'المميزة',price:1299,period:'شهري',color:'#DC2626',badge:'للفنادق الكبيرة',branch_fee:0,branch_enabled:true,pos_devices:0,max_guests:0,
     features:['نزلاء غير محدود','أجهزة نقاط دفع غير محدودة','فرع واحد مدمج','كامل مميزات المتقدمة','لوحة تحليل متقدمة','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي لحظي','API كامل','مدير حساب مخصص','دعم مباشر أولوية قصوى'],
     excluded:[]},
    {id:'enterprise',name:'المؤسسية',price:1999,period:'شهري',color:'#1A56DB',badge:'للسلاسل',branch_fee:200,branch_enabled:true,pos_devices:0,max_guests:0,
     features:['نزلاء غير محدود','أجهزة نقاط دفع غير محدودة','إدارة مركزية لجميع الفروع','200 ر.س لكل فرع إضافي','تقارير موحدة للسلسلة','ذكاء اصطناعي لكامل السلسلة','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي لحظي','API مفتوح كامل','مدير حساب مخصص','دعم على مدار الساعة'],
     excluded:[]},
    {id:'ultimate',name:'الشاملة',price:2999,period:'شهري',color:'#0A3D2E',badge:'⭐ VIP',branch_fee:0,branch_enabled:true,pos_devices:0,max_guests:0,
     features:['نزلاء غير محدود','أجهزة نقاط دفع غير محدودة','فروع غير محدودة بدون رسوم','كامل جميع مميزات المنصة','تخصيص كامل حسب الطلب','تكامل GDS (Amadeus/Sabre)','فواتير VAT 15% وسياحة 2.5%','نسخ احتياطي مزدوج لحظي','API + Webhooks','فريق دعم مخصص','تدريب وإعداد مجاني','SLA 99.9%'],
     excluded:[]},
  ];
}
...
async function saveGitHubSettings(){
  ...
  const branch=document.getElementById('cfg-github-branch')?.value.trim()||'main';
  if(!token||!repo){toast_('أدخل Token والمستودع',false);return;}
  const r=await api('/api/admin/settings/save',{github_token:token,github_repo:repo,github_branch:branch});
  if(r?.ok)toast_('✅ تم حفظ إعدادات GitHub');else toast_(r?.error||'خطأ',false);
}
async function testGitHubConnection(){
  const token=document.getElementById('cfg-github-token')?.value.trim();
  const repo=document.getElementById('cfg-github-repo')?.value.trim();
  const el=document.getElementById('github-test-result');if(!el)return;
  el.style.display='block';el.textContent='⏳ جاري الاختبار...';el.style.color='var(--muted)';
  try{
    ...
    if(res.ok){const d=await res.json();el.textContent=`✅ متصل — ${d.full_name} · الفرع: ${d.default_branch}`;el.style.color='var(--gd)';}
    else{el.textContent='❌ خطأ: '+(res.status===401?'Token غير صحيح':res.status===404?'المستودع غير موجود':res.status);el.style.color='var(--rd)';}
  }catch(e){el.textContent='❌ خطأ في الاتصال';el.style.color='var(--rd)';}
}
async function saveBackupSettings(){
  const token=document.getElementById('cfg-backup-token')?.value.trim();
  const repo=document.getElementById('cfg-backup-repo')?.value.trim();
  if(!token||!repo){toast_('أدخل Token ومستودع النسخ الاحتياطي',false);return;}
  const r=await api('/api/admin/settings/save',{github_backup_token:token,github_backup_repo:repo});
  if(r?.ok)toast_('✅ تم الحفظ — النسخ الاحتياطي سيعمل تلقائياً');else toast_(r?.error||'خطأ',false);
}
async function testBackupNow(){
  const el=document.getElementById('backup-gh-result');if(el){el.style.display='block';el.textContent='⏳ جاري النسخ الاحتياطي...';}
  const r=await api('/api/admin/clients/backup');
  if(el){
    if(r?.ok){el.textContent=`✅ ${r.message}`;el.style.color='var(--gd)';}
    else{el.textContent='❌ '+(r?.error||'خطأ');el.style.color='var(--rd)';}
  }
}
...
//  SETTINGS
// ═══════════════════════════════════════════════
function rSettings(){const s=D.settings;
  // إعدادات AI
  const ck=document.getElementById('cfg-claude-key');if(ck)ck.value=s.claude_key||'';
  const sk=document.getElementById('cfg-share-key');if(sk)sk.checked=s.share_claude_key!==false;
  const ae=document.getElementById('cfg-ai-enabled');if(ae)ae.checked=s.ai_enabled_for_clients!==false;
  const badge=document.getElementById('ai-status-badge');
  if(badge){const hk=!!(s.claude_key||'').trim(),sh=s.share_claude_key!==false,en=s.ai_enabled_for_clients!==false;
    if(!en){badge.textContent='⛔ AI معطّل';badge.style.background='#FEF2F2';badge.style.color='#991B1B';}
    else if(hk&&sh){badge.textContent='✅ مفتاح مشترك';badge.style.background='#ECFDF5';badge.style.color='#065F46';}
    else if(hk){badge.textContent='🔑 مفتاح خاص';badge.style.background='#FFFBEB';badge.style.color='#92400E';}
    else{badge.textContent='⚠️ لا مفتاح';badge.style.background='#FFFBEB';badge.style.color='#92400E';}}
  checkBackupStatus();
  const map={'cfg-owner-name':'owner_name','cfg-owner-email':'owner_email','cfg-owner-phone':'owner_phone','cfg-system-name':'system_name','cfg-bank-name':'bank_name','cfg-bank-iban':'bank_iban','cfg-trial-days':'trial_days'};Object.entries(map).forEach(([id,k])=>{const el=document.getElementById(id);if(el&&s[k]!==undefined)el.value=s[k];});const p=s.prices||{};['cfg-price-free','cfg-price-pro','cfg-price-ent'].forEach((id,i)=>{const el=document.getElementById(id);if(el)el.value={0:p.free||0,1:p.pro||299,2:p.enterprise||799}[i];});}
...
// ── UPDATE — GitHub Push Workflow ────────────────────────────
let selectedFiles = []; // متعدد الملفات
...
function addSelectedFiles(files){
  ...
  files.forEach(f => {
    const safe = f.name.split('/').pop().split('\\').pop();
    if(allowed.includes(safe) && !selectedFiles.find(x=>x.name===safe)){
...
function renderSelectedFiles(){
  ...
  el.innerHTML = selectedFiles.map((f,i) => `
    <div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--bg);border-radius:8px;margin-bottom:5px;border:1px solid var(--border)">
      <span style="font-size:14px">🐍</span>
      <div style="flex:1"><div style="font-size:11px;font-weight:700">${f.name}</div><div style="font-size:10px;color:var(--muted)">${(f.size/1024).toFixed(1)} KB</div></div>
      <button onclick="removeFile(${i})" style="border:none;background:none;cursor:pointer;color:var(--r);font-size:16px;padding:0">×</button>
    </div>`).join('');
}
function removeFile(i){ selectedFiles.splice(i,1); renderSelectedFiles(); }

// ── تنزيل بيانات العملاء ──────────────────────────────────
async function downloadAdminStore(){
  const btn = event?.target;
  const st  = document.getElementById('backup-status');
  if(btn){ btn.textContent='⏳ جارٍ التحضير...'; btn.disabled=true; }
  if(st) st.textContent = '';
  try{
    const r = await api('/api/admin/store/export', {});
    if(!r.ok){ toast_('خطأ في التصدير', false); return; }
    ...
    a.href     = url;
    a.download = `dheuof_admin_backup_${ts}.json`;
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    URL.revokeObjectURL(url);
    if(st) st.innerHTML = `<span style="color:var(--g);font-weight:700">✅ تم تنزيل النسخة — ${new Date().toLocaleTimeString('ar-SA')}</span>`;
    toast_('✅ تم تنزيل بيانات العملاء — احفظها في مكان آمن');
  } catch(e){
    toast_('خطأ: ' + e.message, false);
  } finally {
    if(btn){ btn.textContent='⬇️ تنزيل بيانات العملاء الآن'; btn.disabled=false; }
  }
}

// ── رفع لـ GitHub ─────────────────────────────────────────
async function pushToGitHub(){
  if(!selectedFiles.length){ toast_('اختر الملفات أولاً', false); return; }
  const btn  = document.getElementById('github-push-btn');
  const prog = document.getElementById('push-progress');
  const desc = document.getElementById('update-desc')?.value || 'تحديث dheuof';
  if(btn){ btn.textContent='⏳ جارٍ الرفع...'; btn.disabled=true; }
  if(prog){ prog.style.display='block'; prog.innerHTML='<div style="font-size:11px;color:var(--muted)">⏳ جارٍ قراءة الملفات وفحص الصياغة...</div>'; }

  try{
    // قراءة كل الملفات
    const filesData = await Promise.all(selectedFiles.map(f => new Promise((res,rej) => {
      const reader = new FileReader();
      reader.onload = e => res({ name: f.name, content_b64: e.target.result.split(',')[1] });
      reader.onerror = rej;
      reader.readAsDataURL(f);
    })));

    if(prog) prog.innerHTML='<div style="font-size:11px;color:var(--muted)">🚀 جارٍ الرفع لـ GitHub...</div>';

    const r = await api('/api/admin/update/github-push', { files: filesData, description: desc });

    if(r.ok){
      const rows = (r.results||[]).map(x =>
        `<div style="display:flex;align-items:center;gap:8px;padding:5px 0;font-size:11px">
          <span>${x.ok?'✅':'❌'}</span>
          <span style="font-weight:700">${x.file}</span>
          <span style="color:var(--muted)">${x.ok?'رُفع بنجاح':x.error||'فشل'}</span>
        </div>`).join('');
      if(prog) prog.innerHTML=`
        <div style="background:var(--gl);border-radius:10px;padding:12px;border:1px solid #6EE7B7">
          <div style="font-size:13px;font-weight:700;color:var(--gd);margin-bottom:8px">✅ ${r.message}</div>
          ${rows}
          <div style="margin-top:8px;padding:7px 10px;background:rgba(4,120,87,.1);border-radius:7px;font-size:11px;color:var(--gd)">
            ⏱ Railway يسحب التحديث الآن — انتظر دقيقة ثم أعد تحميل www.dheuof.com
          </div>
        </div>`;
      selectedFiles = []; renderSelectedFiles();
      loadUpdateHistory();
      toast_('✅ رُفع لـ GitHub — Railway يعيد النشر');
    } else {
      const rows = (r.results||[]).map(x =>
        `<div style="font-size:11px;padding:3px 0;color:var(--r)">${x.file}: ${x.error||'فشل'}</div>`).join('');
      if(prog) prog.innerHTML=`<div style="background:var(--rl);border-radius:10px;padding:10px;border:1px solid #FCA5A5"><div style="font-size:12px;font-weight:700;color:var(--rd)">❌ ${r.message||r.error}</div>${rows}</div>`;
      toast_(r.error||'فشل الرفع', false);
    }
  } catch(e){
    if(prog) prog.innerHTML=`<div style="color:var(--r);font-size:11px">❌ خطأ: ${e.message}</div>`;
    toast_('خطأ: ' + e.message, false);
  } finally {
    if(btn){ btn.textContent='🚀 رفع لـ GitHub الآن'; btn.disabled=false; }
  }
}
...
async function loadUpdateHistory(){
  ...
  el.innerHTML = [...history].reverse().map(h => `
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:0.5px solid var(--border)">
      <div style="font-size:18px">${h.source==='github_push'?'🚀':'🐍'}</div>
      <div style="flex:1;min-width:0">
        <div style="font-size:11px;font-weight:700;color:var(--text)">${h.file}</div>
...
async function logout(){try{await api('/api/admin/logout',{});}catch(e){}document.cookie='admin_token=; Path=/; Max-Age=0';location.href='/admin';}

async function saveAISettings(){
  const key=(document.getElementById('cfg-claude-key')||{}).value||'';
  const share=(document.getElementById('cfg-share-key')||{}).checked!==false;
  const enabled=(document.getElementById('cfg-ai-enabled')||{}).checked!==false;
  const r=await api('/api/admin/settings/ai',{claude_key:key,share_claude_key:share,ai_enabled_for_clients:enabled});
  const msg=document.getElementById('ai-save-msg');
  if(r.ok){
    D.settings.claude_key=key;D.settings.share_claude_key=share;D.settings.ai_enabled_for_clients=enabled;
    if(msg){msg.textContent='✅ تم حفظ إعدادات AI';msg.style.color='var(--g)';msg.style.display='block';setTimeout(()=>msg.style.display='none',3000);}
    rSettings();toast_('تم حفظ إعدادات AI ✓');
  }else{
    if(msg){msg.textContent='❌ '+(r.error||'خطأ');msg.style.color='var(--r)';msg.style.display='block';}
    toast_(r.error||'فشل',false);
  }
}
async function testClaudeKey(){
  const key=(document.getElementById('cfg-claude-key')||{}).value||''
              ||(document.getElementById('ai-key-input')||{}).value||'';
  const res=document.getElementById('cfg-key-test-result');
  if(!key){if(res){res.textContent='أدخل المفتاح أولاً';res.style.color='var(--r)';res.style.display='block';}return;}
  if(res){res.textContent='⏳ جارٍ الاختبار...';res.style.color='var(--muted)';res.style.display='block';}
  try{
    const r=await api('/api/admin/settings/ai/test',{claude_key:key,key:key});
    if(res){res.textContent=r.ok?'✅ المفتاح صحيح ✓':'❌ '+(r.error||'غير صحيح');res.style.color=r.ok?'var(--g)':'var(--r)';}
  }catch(e){if(res){res.textContent='❌ خطأ في الاتصال';res.style.color='var(--r)';}}
}
async function checkBackupStatus(){
  const r=await api('/api/admin/backup/status');
  const badge=document.getElementById('backup-badge');
  const res=document.getElementById('backup-result');
  if(badge){badge.textContent=r.configured?'✅ مُفعّل':'⚠️ غير مُفعّل';
    badge.style.background=r.configured?'#ECFDF5':'#FFFBEB';
    badge.style.color=r.configured?'#065F46':'#92400E';}
  if(res){res.style.display='block';
    res.innerHTML=`<div style="padding:8px 10px;background:var(--bg);border-radius:8px;border:0.5px solid var(--border);font-size:11px">
      <div>الـ Repo: <b>${r.repo||'غير محدد'}</b></div>
      <div>عملاء محفوظون: <b>${r.clients_count||0}</b></div>
      <div style="color:var(--muted);font-size:10px;margin-top:4px">${r.message||''}</div>
    </div>`;}
}
async function restoreBackup(){
  const res=document.getElementById('backup-result');
  if(res){res.style.display='block';res.innerHTML='<div style="color:var(--muted)">⏳ جارٍ الاسترجاع...</div>';}
  const r=await api('/api/admin/backup/restore');
  if(res){res.innerHTML=`<div style="padding:8px 10px;background:${r.ok?'#ECFDF5':'#FEF2F2'};border-radius:8px;color:${r.ok?'#065F46':'#991B1B'};font-size:11px">${r.ok?'✅':'❌'} ${r.message||''}</div>`;}
  if(r.ok){await boot();toast_(`✅ ${r.message}`);}
  else toast_(r.message||'فشل',false);
}
...
if __name__ == "__main__":
    ...
    print(f"  كلمة المرور: [مُشفَّرة — راجع ADMIN_PASS_HASH]")

  ...
  <div class="content">
    ...
    <div class="pg" id="pg-push">
      <div class="card cb" style="margin-bottom:12px">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:var(--pl);color:var(--p)">📢</div>إشعار داخل النظام</div></div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:10px">الرسالة تظهر في لوحة العميل عند دخوله التالي.</div>
        <div class="fg"><label>الرسالة</label><textarea id="push-msg" rows="3" placeholder="مثال: تم تحديث النظام — جربوا ميزة تحليل السوق الجديدة!"></textarea></div>
        ...
        <button class="btn bp" onclick="sendPush()">📤 إرسال الإشعار</button>
        <div id="push-result" style="margin-top:8px;font-size:11px;color:var(--g);display:none"></div>
      </div>
      <div class="card cb" style="margin-bottom:12px">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:#25D36622;color:#25D366">📱</div>رسالة واتساب جماعية</div></div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:10px">يفتح محادثة واتساب لكل عميل عنده جوال.</div>
        <div class="fg"><label>نص الرسالة</label><textarea id="wa-msg" rows="4" placeholder="أهلاً، نود إعلامك بأن..."></textarea></div>
        <div class="fg"><label>المستهدفون</label>
          <select id="wa-target">
            <option value="all">جميع العملاء</option>
            <option value="trial">العملاء في التجربة</option>
            <option value="active">العملاء النشطون</option>
            <option value="nologin">سجّلوا ولم يدخلوا</option>
          </select>
        </div>
        <button class="btn" style="background:#25D366;color:#fff;border:none" onclick="openWhatsappList()">📱 فتح قائمة واتساب</button>
        <div id="wa-list" style="margin-top:10px"></div>
      </div>
      <div class="card cb">
        <div class="ch"><div class="ch-l"><div class="cico" style="background:var(--gl);color:var(--g)">📊</div>تصدير بيانات العملاء</div></div>
        <div style="font-size:11px;color:var(--muted);margin-bottom:10px">تصدير CSV يفتح في Excel — الاسم، الجوال، الإيميل، المدينة، الخطة، الحالة.</div>
        <div class="fg"><label>تصدير</label>
          <select id="csv-target">
            <option value="all">جميع العملاء</option>
            <option value="trial">العملاء في التجربة</option>
            <option value="active">العملاء النشطون</option>
            <option value="nologin">سجّلوا ولم يدخلوا</option>
        ...
        <button class="btn" style="background:var(--g);color:#fff;border:none" onclick="exportCSV()">⬇️ تحميل CSV</button>
      </div>
    </div>
    ...
    <div class="pg" id="pg-clients">
      <div class="card cb">
        ...
        <div style="overflow-x:auto"><table class="tbl">
          <thead><tr><th>العميل</th><th>المدينة</th><th>الخطة</th><th>الحالة</th><th>أيام الاستخدام</th><th>آخر دخول</th><th>انتهاء التجربة</th><th>الإيراد</th><th>إجراءات</th></tr></thead>
...
function renderOnline(an){
  ...
  el.innerHTML=sess.map((s,i)=>{
    ...
    const stat=cs[String(s.cid)]||{};
    const name=c?(c.hotel_name||c.name):(stat.hotel_name||stat.name||'عميل غير معروف');
    const sub=c?(c.name&&c.name!==c.hotel_name?c.name:''):'';
    const mins=Math.round((new Date()-new Date(s.last_seen))/60000);
    const clr=colors[i%colors.length];
    return`<div class="online-item"><div class="online-av" style="background:${clr}22;color:${clr}">${name.charAt(0)}</div><div class="online-dot"></div><div class="online-info"><div class="online-name">${name}${sub?` <span style="font-size:9px;color:var(--muted);font-weight:400">(${sub})</span>`:''}</div><div class="online-meta">آخر نشاط: ${mins===0?'الآن':mins+' دقيقة'} · ${s.pages||0} صفحات · ${s.actions||0} أحداث</div></div></div>`;
  }).join('');
}
...
function _getTargetClients(targetKey){
  const cs=analytics_data.client_stats||{};
  if(targetKey==='trial')   return D.clients.filter(c=>c.status==='trial');
  if(targetKey==='active')  return D.clients.filter(c=>c.status==='active');
  if(targetKey==='nologin') return D.clients.filter(c=>c.status==='trial'&&!((cs[String(c.id)]||{}).last_login||''));
  return D.clients;
}

function openWhatsappList(){
  const msg=gv('wa-msg').trim();
  if(!msg){toast_('أدخل نص الرسالة أولاً',false);return;}
  const target=gv('wa-target');
  const list=_getTargetClients(target).filter(c=>c.phone);
  const el=document.getElementById('wa-list');
  if(!list.length){el.innerHTML='<div style="font-size:11px;color:var(--r);padding:8px">لا يوجد عملاء بجوال مسجّل في هذه الفئة</div>';return;}
  el.innerHTML='<div style="font-size:11px;color:var(--muted);margin-bottom:8px">انسخ الرسالة ثم اضغط على كل عميل:</div>'+
    '<div style="background:#0F172A;color:#10B981;font-family:monospace;font-size:11px;padding:10px;border-radius:8px;margin-bottom:10px;white-space:pre-wrap;cursor:pointer" onclick="navigator.clipboard.writeText(this.textContent);toast_('تم نسخ الرسالة ✓')">'+msg+'</div>'+
    '<div style="display:flex;flex-direction:column;gap:6px">'+
    list.map(function(c){
      var phone='966'+c.phone.replace(/^0/,'');
      var waMsg=encodeURIComponent(msg);
      return '<a href="https://wa.me/'+phone+'?text='+waMsg+'" target="_blank"'+
        ' style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:#F0FDF4;border:1px solid #86EFAC;border-radius:8px;text-decoration:none;color:#166534">'+
        '<div style="width:32px;height:32px;border-radius:50%;background:#25D366;color:#fff;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;flex-shrink:0">'+(c.hotel_name||c.name).charAt(0)+'</div>'+
        '<div style="flex:1"><div style="font-size:12px;font-weight:700">'+(c.hotel_name||c.name)+'</div>'+
        '<div style="font-size:10px;color:#6B7280">'+c.phone+' · '+(c.city||'—')+' · '+(c.status==='trial'?'تجربة':'نشط')+'</div></div>'+
        '<div style="font-size:11px;font-weight:700;color:#25D366">فتح ↗</div></a>';
    }).join('')+
    '</div>'+
    '<div style="margin-top:8px;font-size:10px;color:var(--muted)">✓ '+list.length+' عميل بجوال مسجّل</div>';
}

function exportCSV(){
  var target=gv('csv-target');
  var list=_getTargetClients(target);
  if(!list.length){toast_('لا يوجد عملاء في هذه الفئة',false);return;}
  var headers=['الاسم','اسم المنشأة','الجوال','البريد الإلكتروني','المدينة','نوع المنشأة','الخطة','الحالة','تاريخ التسجيل','انتهاء التجربة'];
  var rows=list.map(function(c){
    return [c.name,c.hotel_name,c.phone,c.email,c.city,c.hotel_type,c.plan,c.status,(c.created_at||'').split('T')[0],c.trial_end]
...
');
  ...
  toast_('تم تصدير '+list.length+' عميل ✓');
}
...
function showClientDetail(id){
  ...
  document.getElementById('modal-client-body').innerHTML=`
    ...
    <div class="g2">
      ...
      <div><div style="font-size:10px;color:var(--muted)">آخر ظهور</div><div style="font-size:11px;margin-top:2px;direction:ltr;text-align:right">${cs.last_seen_human||'—'}</div></div>
      <div><div style="font-size:10px;color:var(--muted)">آخر صفحة</div><div style="font-size:11px;margin-top:2px">${cs.last_page||'—'}</div></div>
    </div>
    ${(cs.login_history&&cs.login_history.length)?`
    <div style="margin-top:10px">
      <div style="font-size:11px;font-weight:700;color:var(--text);margin-bottom:6px">📅 سجل الدخول (${cs.login_history.length} دخول)</div>
      <div style="max-height:160px;overflow-y:auto;display:flex;flex-direction:column;gap:3px">
        ${[...cs.login_history].reverse().map((l,i)=>`
          <div style="display:flex;justify-content:space-between;align-items:center;padding:5px 8px;background:${i%2===0?'var(--bg)":'var(--bg2)'};border-radius:6px;font-size:10px">
            <span style="color:var(--muted)">#${cs.login_history.length-i}</span>
            <span style="font-weight:600;direction:ltr">${l.human||l.time||'—'}</span>
...
async function uploadFile(){
  ...
  try{
    ...
    reader.onload = async(e) => {
      ...
      loadUpdateHistory();  // removed await - not in async context

...
class AdminHandler(BaseHTTPRequestHandler):
    ...
    def _p_settings_save(self, b):
        ...
        for k in ["owner_name","owner_email","owner_phone","owner_whatsapp","contact_message","system_name","bank_iban","bank_name","trial_days","prices","plans","ai_market_competitors","logo_emoji","primary_color","announcement","disabled_pages"]:
            if k in b: s[k] = b[k]
    ...
    <div class="pg" id="pg-settings">
      <div class="card cb">
        ...
        <div class="g2">
          ...
          <div class="fg"><label>أيقونة النظام (emoji)</label><input id="cfg-logo-emoji" placeholder="🏨" maxlength="4"/></div>
          <div class="fg"><label>اللون الرئيسي للعميل</label><div style="display:flex;gap:8px;align-items:center"><input type="color" id="cfg-primary-color" value="#185FA5" style="width:48px;height:36px;border:none;cursor:pointer;border-radius:6px"/><input id="cfg-primary-color-hex" placeholder="#185FA5" style="flex:1" oninput="document.getElementById('cfg-primary-color').value=this.value"/></div></div>
          <div class="fg"><label>إعلان عام للعملاء (يظهر كشريط في الأعلى)</label><input id="cfg-announcement" placeholder="مثال: سيكون النظام في صيانة الجمعة 12-2 ص"/></div>
          <div class="fg"><label>صفحات مقفولة للعملاء <span style="font-size:9px;color:var(--muted)">(اختر ما تريد إخفاءه)</span></label>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:4px">
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-kpi"/> مؤشرات KPI</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-ai"/> مساعد AI</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-pms"/> تكامل PMS</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-budget"/> الميزانية</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-cashflow"/> التدفق النقدي</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-match"/> المطابقة اليومية</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-reports"/> التقرير اليومي</label>
              <label style="display:flex;align-items:center;gap:6px;font-size:11px"><input type="checkbox" id="dp-backup"/> النسخ الاحتياطي</label>
            </div>
          </div>
        </div>
...
function rSettings(){
  const s=D.settings;
  const map={'cfg-owner-name':'owner_name','cfg-owner-email':'owner_email','cfg-owner-phone':'owner_phone','cfg-system-name':'system_name','cfg-bank-name':'bank_name','cfg-bank-iban':'bank_iban','cfg-trial-days':'trial_days','cfg-logo-emoji':'logo_emoji','cfg-primary-color-hex':'primary_color','cfg-announcement':'announcement'};
  Object.entries(map).forEach(([id,k])=>{const el=document.getElementById(id);if(el&&s[k]!==undefined)el.value=s[k];});
  const pc=document.getElementById('cfg-primary-color');if(pc&&s.primary_color)pc.value=s.primary_color;
  const p=s.prices||{};['cfg-price-free','cfg-price-pro','cfg-price-ent'].forEach((id,i)=>{const el=document.getElementById(id);if(el)el.value={0:p.free||0,1:p.pro||299,2:p.enterprise||799}[i];});
  const dp=s.disabled_pages||[];['kpi','ai','pms','budget','cashflow','match','reports','backup'].forEach(pg=>{const el=document.getElementById('dp-'+pg);if(el)el.checked=dp.includes(pg);});
...
async function saveSettings(){
  const disabled_pages=['kpi','ai','pms','budget','cashflow','match','reports','backup'].filter(pg=>{const el=document.getElementById('dp-'+pg);return el&&el.checked;});
  const primary_color=gv('cfg-primary-color-hex')||document.getElementById('cfg-primary-color')?.value||'#185FA5';
  const r=await api('/api/admin/settings/save',{
    owner_name:gv('cfg-owner-name'),owner_email:gv('cfg-owner-email'),owner_phone:gv('cfg-owner-phone'),
    system_name:gv('cfg-system-name'),bank_name:gv('cfg-bank-name'),bank_iban:gv('cfg-bank-iban'),
    trial_days:parseInt(gv('cfg-trial-days')||15),
    prices:{free:0,pro:parseFloat(gv('cfg-price-pro')||299),enterprise:parseFloat(gv('cfg-price-ent')||799)},
    logo_emoji:gv('cfg-logo-emoji')||'🏨',
    primary_color,
    announcement:gv('cfg-announcement')||'',
    disabled_pages,
    owner_whatsapp:gv('cfg-whatsapp')||'',
    contact_message:document.getElementById('cfg-contact-msg')?.value||'',
  });
  if(r.ok){
    Object.assign(D.settings,{owner_name:gv('cfg-owner-name'),owner_email:gv('cfg-owner-email'),system_name:gv('cfg-system-name'),primary_color,disabled_pages,logo_emoji:gv('cfg-logo-emoji')});
    ss('sb-owner-name',gv('cfg-owner-name'));ss('sb-owner-email',gv('cfg-owner-email'));ss('sb-sys-name',gv('cfg-system-name'));
...
async function logout(){try{await api('/api/admin/logout',{});}catch(e){}location.href='/admin';}
</script>
</body>
</html>"""

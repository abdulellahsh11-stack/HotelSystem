...
class ClientHandler(BaseH):
    ...
    def _p_market_ai_client(self, b, store):
        ...
        s = adm.get("admin_settings", {})
        key = store.get_d("settings",{}).get("claude_key","") or s.get("claude_key","")
        city = b.get("city", store.get_d("settings",{}).get("city","riyadh"))
        htype = b.get("type", store.get_d("settings",{}).get("hotel_type","hotel"))
        
        # Static fallback market data per city
        rates = {
        ...
        r = rates.get(city, {"hotel_avg":350,"apart_avg":250,"hotel_occ":65,"apart_occ":60,"season":"موسم اعتيادي"})
        
        if key:
            try:
                prompt = f"أنت خبير فندقي سعودي. أعطني تحليل السوق الفندقي في {city} الآن بـ JSON فقط: hotel_avg, apart_avg, hotel_occ, apart_occ, season, season_factor, recommendation, insight, source"
                payload = json.dumps({"model":"claude-sonnet-4-5","max_tokens":400,"messages":[{"role":"user","content":prompt}]}).encode()
                ...
                data = json.loads(_ur.urlopen(req,timeout=20).read())
                text = data["content"][0]["text"].strip()
                import re as _re
                m = _re.search(r'\{.*\}', text, _re.DOTALL)
                if m:
                    r = {**r, **json.loads(m.group(0))}
                    r["source"] = "Claude AI"
            except Exception as e:
                r["source"] = f"بيانات ثابتة (خطأ API: {str(e)[:50]})"
        else:
            r["source"] = "بيانات ثابتة"
    ...
    def _p_market_ws_client(self, b, store):
        """بحث ويب + AI لأسعار السوق"""
        self._p_market_ai_client(b, store)
...
{{"performance_score": 75, "performance_label": "جيد", "strengths": ["..."], "weaknesses": ["..."], "recommendations": [{{"title":"...","action":"...","impact":"..."}}], "revenue_forecast": {{"current":0,"improved":0,"improvement_pct":0}}, "pricing_advice": "...", "market_comparison": "...", "summary": "..."}}"""
        ...
        if key:
            try:
                ...
                data = json.loads(_ur.urlopen(req, timeout=25).read())
                text = data["content"][0]["text"].strip()
                m = _re.search(r'\{.*\}', text, _re.DOTALL)
                if m:
                ...
                result["source"] = f"بيانات محسوبة (خطأ API: {str(e)[:60]})"
...
<title>{sn} — برنامج إدارة الفنادق</title>
...
<nav><div class="nav-inner">
  <div class="nav-logo">{em} <span style="color:var(--gold);font-weight:900;font-size:20px;">{sn}</span> <span>برنامج إدارة الفنادق</span></div>
  <div class="nav-links">
...
<section class="hero" id="hero">
  <div class="hero-inner">
    <div class="hero-text fade">
      <h1>استمتع بإدارة مالية متكاملة لفندقك مع برنامج<em><span style="color:var(--gold)">{sn}</span> — نظام إدارة الفنادق المتطور!</em></h1>
...
<section class="section" id="features">
  <div class="sec-inner">
    ...
    <div class="feat-grid">
      <div class="feat-card fade"><div class="feat-icon" style="background:#0a1628">🔒</div><div class="feat-body"><h3>أمان وحماية البيانات</h3><p>نضمن حماية بياناتك المالية بتقنيات أمان متطورة مما يمنحك راحة البال.</p><button class="feat-btn" onclick="document.getElementById('login').scrollIntoView({{behavior:'smooth'}})">تأمين بياناتك</button></div></div>
      <div class="feat-card fade" style="transition-delay:.1s"><div class="feat-icon" style="background:#0f1f0f">💳</div><div class="feat-body"><h3>تكامل مع الأنظمة الأخرى</h3><p>دمج البرنامج مع أنظمة الدفع الإلكتروني وإدارة العمليات المالية بشكل متكامل.</p><button class="feat-btn" onclick="document.getElementById('login').scrollIntoView({{behavior:'smooth'}})">تعرف على التكامل</button></div></div>
      <div class="feat-card fade" style="transition-delay:.2s"><div class="feat-icon" style="background:#1a1a2e">📱</div><div class="feat-body"><h3>واجهة سهلة الاستخدام</h3><p>تصميم بسيط وسهل يتيح لجميع الموظفين استخدامه بكفاءة وسرعة.</p><button class="feat-btn" onclick="document.getElementById('login').scrollIntoView({{behavior:'smooth'}})">استمتع بالتجربة</button></div></div>
    </div>
  </div>
</section>
  ...
  <div class="login-inner">
    <div class="login-text fade">
      ...
      <div class="perks">
        ...
        <div class="perk"><div class="perk-ic">🏨</div><span>يعمل مع الفنادق والشقق والاستراحات</span></div>
      </div>
    </div>
...
<footer><div style="margin-bottom:8px"><a href="#hero">الرئيسية</a><a href="#features">الخدمات</a><a href="#login">تواصل</a></div>© 2026 {sn} — جميع الحقوق محفوظة</footer>

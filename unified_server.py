...
class ClientHandler(BaseH):
    ...
    def _p_market_ai_client(self, b, store):
        ...
        key = client_key or admin_key if ai_enabled else ""
    ...
    def _p_market_ws_client(self, b, store):
        ...
        result = {
            **base,
            ...
            "season": "موسم اعتيادي", "season_factor": 1.2,
...
document.querySelectorAll('.fade').forEach(el=>obs.observe(el));
    ...
    def _admin_test_claude_key(self, b):
        ...
        key = b.get("claude_key","").strip()
        if not key:
            self._json({"ok":False,"error":"أدخل المفتاح"}); return
        text, err = _claude_call(key, "قل: مرحبا", max_tokens=10, timeout=15)
        if text:
            self._json({"ok":True,"model":"claude-sonnet-4-5","message":"المفتاح يعمل ✓"})
        else:
            self._json({"ok":False,"error": err or "المفتاح غير صحيح أو منتهي الصلاحية"})

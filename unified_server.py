...
def _github_restore():
    ...
    try:
        ...
        # دائماً استرجع admin_settings من GitHub إذا كانت غنية أكثر
        gh_settings = gh_data.get("admin_settings", {})
        loc_settings = local_data.get("admin_settings", {})
        if gh_settings:
            # دمج: البيانات المحلية تأخذ الأولوية إلا إذا كانت فارغة
            merged_settings = {**gh_settings, **{k:v for k,v in loc_settings.items() if v}}
            local_data["admin_settings"] = merged_settings
        # استرجع license_keys و payments أيضاً
        if gh_data.get("license_keys"):
            local_data.setdefault("license_keys", gh_data["license_keys"])
        if gh_data.get("payments"):
            local_data.setdefault("payments", gh_data["payments"])
...
# ── الاسترجاع يكتمل أولاً قبل تحميل AdminStore (يمنع race condition) ──
_t = threading.Thread(target=_github_restore, daemon=True)
_t.start()
_t.join(timeout=15)  # انتظر حتى 15 ثانية ثم تابع

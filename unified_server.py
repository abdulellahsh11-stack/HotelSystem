...
<div class="pg" id="pg-register">
  ...
  <div class="fg"><label>المدينة</label>
    <select id="r-city"><option value="riyadh">الرياض</option><option value="jeddah">جدة</option><option value="makkah">مكة</option><option value="madinah">المدينة</option><option value="dammam">الدمام</option><option value="abha">أبها</option><option value="taif">الطائف</option></select>
  </div>
...
function showErr(id,msg){const e=document.getElementById(id);e.textContent=msg;e.style.display='block';setTimeout(()=>e.style.display='none',4000);}
    ...
    def _build_client_html(self,client,store):
        # Import main.py HTML but inject subscription & ticket components
        try:
            import importlib.util
            # Load main.py _build_html if available
            spec = importlib.util.spec_from_file_location("hotel_main",
                os.path.join(os.path.dirname(os.path.abspath(__file__)),"main.py"))
            if spec:
                mod = importlib.util.load_module_from_spec(spec)  # type: ignore
        except: pass
        # Return notice with link to main system
        name = client["hotel_name"] if client else "النظام التجريبي"
        trial = client.get("trial_end","—") if client else "—"
        dl    = days_left(client.get("trial_end")) if client else None
        return f"""<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"/>
<meta http-equiv="refresh" content="0; url=http://127.0.0.1:5050"/>
<title>تحميل...</title></head><body>
<p style="font-family:system-ui;text-align:center;padding:40px;direction:rtl">
  مرحباً بك في {name} — جارٍ التحميل...<br/>
  <a href="http://127.0.0.1:5050">اضغط هنا إن لم يفتح تلقائياً</a>
</p></body></html>"""

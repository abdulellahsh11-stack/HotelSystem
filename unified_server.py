...
{{"performance_score": 75, "performance_label": "جيد", "strengths": ["..."], "weaknesses": ["..."], "recommendations": [{{"title":"...","action":"...","impact":"..."}}], "revenue_forecast": {{"current":0,"improved":0,"improvement_pct":0}}, "pricing_advice": "...", "market_comparison": "...", "summary": "..."}}"""
    ...
    def _p_client_select_plan(self,b,store,client):
        """العميل يختار خطة — يفتح تذكرة تلقائية للأدمن"""
        if not client:
        ...
        whatsapp  = s.get("owner_whatsapp","")
        # ── فتح تذكرة تلقائية في لوحة الأدمن ──
        ticket = {
            "id":          _id(),
            ...
            "messages": [{
                ...
                "text": (
                    f"الفندق: {cname}\n"
                    f"البريد: {cemail}\n"
                    f"الخطة المطلوبة: {plan_name}\n"
                    f"السعر: {price} ر.س / شهر\n"
                    f"الإجراء: في انتظار تأكيد التحويل البنكي وإرسال كود التفعيل"
                ),
                "time": _iso(),
                "auto": True,
            }],
        ...
        log.info(f"Plan selected: client={cname} plan={plan_name}")
        self._json({
            "ok":       True,
            "ticket_id": ticket["id"],
            "bank": {
...
إذا كان الأمر يتطلب تنفيذاً، اذكر الإجراءات بدقة."""
    ...
    def _api_payment_plans(self):
        ...
        bank = {
            # يدعم الأسماء القديمة والجديدة معاً

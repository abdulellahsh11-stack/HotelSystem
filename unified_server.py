...
_MECCA_OFFSET = 3  # UTC+3

def _mecca_now():
    """الوقت الحالي بتوقيت مكة"""
    return datetime.utcnow() + timedelta(hours=_MECCA_OFFSET)

def _today():
    """تاريخ اليوم بتوقيت مكة"""
    return _mecca_now().strftime("%Y-%m-%d")

def _iso():
    """ISO timestamp بتوقيت مكة"""
    return _mecca_now().strftime("%Y-%m-%dT%H:%M:%S") + "+03:00"

def _iso_mecca_human():
    """وقت مكة بشكل مقروء: 2026-05-03 14:32"""
    return _mecca_now().strftime("%Y-%m-%d %H:%M")
...
class AnalyticsEngine:
    ...
    def session_open(self, token, cid, hotel_name="", name=""):
        with self._lock:
            now_iso   = _iso()
            now_human = _iso_mecca_human()
            self.sessions[token] = {
                "cid": str(cid), "start": now_iso,
                "last_seen": now_iso, "pages": 0, "actions": 0,
                "ip": "—",
            }
            self._inc_daily("logins")
            cs = self.client_stats.setdefault(str(cid), {
                "days_active":  set(),
                "total_actions":0,
                "last_page":    "—",
                "errors":       0,
            ...
            cs["last_login"]      = now_iso
            cs["last_seen_human"] = now_human
            if hotel_name: cs["hotel_name"] = hotel_name
            if name:       cs["name"]       = name
            # سجل الدخول التراكمي — آخر 100 دخول
            cs.setdefault("login_history", []).append({
                "time":  now_iso,
                "human": now_human,
                "date":  _today()
            })
            cs["login_history"] = cs["login_history"][-100:]
    ...
    def track_page(self, token, page):
        with self._lock:
            if token in self.sessions:
                ...
                cs = self.client_stats.setdefault(str(cid),{"days_active":set(),"total_actions":0,"last_page":"—","errors":0,"first_seen":_today(),"last_login":"","last_seen_human":"","hotel_name":"","name":"","login_history":[]})
                cs["last_page"]       = page
                cs["last_seen_human"] = now_human
                cs["days_active"].add(_today())
    ...
    def snapshot(self):
        with self._lock:
            ...
            return {
                ...
                "client_stats":     {k:{"days_active":len(v["days_active"]),"total_actions":v["total_actions"],"last_page":v["last_page"],"errors":v["errors"],"first_seen":v.get("first_seen",""),"last_login":v.get("last_login",""),"last_seen_human":v.get("last_seen_human",""),"hotel_name":v.get("hotel_name",""),"name":v.get("name",""),"login_history":v.get("login_history",[])} for k,v in self.client_stats.items()},
...
class ClientHandler(BaseH):
    ...
    def _p_client_login(self,b):
        ...
        adm.set("clients",clients)
        analytics.session_open(tok, client["id"], hotel_name=client.get("hotel_name",""), name=client.get("name",""))
        self.send_response(200)
        self.send_header("Content-Type","application/json; charset=utf-8")
...
#  تشغيل الخادمين
# ══════════════════════════════════════════════════════════════
class UnifiedHandler(BaseH):
    """
    Handler واحد على PORT واحد — Railway compatible
    /admin  → AdminHandler
    /*      → ClientHandler
    """
    _admin_h = None
    ...
    def do_GET(self):
        self._get_handler().do_GET(self)

    def do_POST(self):
        self._get_handler().do_POST(self)

    def do_OPTIONS(self):
        try:
            self._get_handler().do_OPTIONS(self)
...
def main():
    """نقطة الدخول — PORT واحد لكل شيء (Railway compatible)"""
    port = CLIENT_PORT   # Railway يعطي PORT واحد فقط
    print("="*58)
    print("  نظام إدارة الفنادق — Unified SaaS Server")
    print("="*58)
    print(f"  PORT الموحّد  → {port}")
    print(f"  لوحة العميل  → :{port}/")
    print(f"  لوحة المالك  → :{port}/admin")
    print(f"  Admin Pass   → {ADMIN_PASS}")
    print("="*58)
    sv = HTTPServer(("0.0.0.0", port), UnifiedHandler)
    log.info(f"✓ Unified server ready on port {port} (client + admin)")
    try:
        sv.serve_forever()

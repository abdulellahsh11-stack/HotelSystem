...
# كلمة المرور مُشفّرة — القيمة الافتراضية = hash لـ Admin@2025#Hotel
_ADMIN_HASH = os.environ.get(
    "ADMIN_PASS_HASH",
    "a2b3683a64ba220757e826c1e1efd1943fec66d6d9763e7ff37c408437fd5d34"
)
# للتوافق مع أي كود قديم يقرأ ADMIN_PASS (يُحذف لاحقاً)
ADMIN_PASS  = "***مُشفَّر***"
    ...
    try:
        ...
        safe = {
            "clients": [{
                ...
                "phone":        c.get("phone",""),
                "city":         c.get("city",""),
                "hotel_type":   c.get("hotel_type",""),
                "plan":         c.get("plan",""),
                "status":       c.get("status",""),
                "trial_start":  c.get("trial_start",""),
            ...
            "tickets":        store_data.get("tickets", []),
...
def _github_restore():
    ...
    try:
        ...
        gh_clients  = {str(c["id"]): c for c in gh_data.get("clients", []) if c.get("id")}
        loc_clients = {str(c["id"]): c for c in local_data.get("clients", []) if c.get("id")}
        # IDs المحذوفة — لا تعود أبداً
        deleted_ids = set(str(x) for x in local_data.get("deleted_client_ids", []))
        deleted_ids |= set(str(x) for x in gh_data.get("deleted_client_ids", []))
        merged = {}
        for cid, gc in gh_clients.items():
            if cid in deleted_ids: continue  # تجاهل المحذوفين
        ...
        local_data["deleted_client_ids"] = list(deleted_ids)
        # دائماً ادمج admin_settings — GitHub يأخذ الأولوية على الافتراضيات
        gh_settings  = gh_data.get("admin_settings", {})
        loc_settings = local_data.get("admin_settings", {})
        if gh_settings:
            local_data["admin_settings"] = {**gh_settings, **{k:v for k,v in loc_settings.items() if v}}
        # استرجع license_keys و payments و tickets
        for _k in ("license_keys", "payments", "tickets"):
            if gh_data.get(_k) and not local_data.get(_k):
                local_data[_k] = gh_data[_k]
...
# ── CRITICAL: انتظار اكتمال الاسترجاع قبل تحميل AdminStore ──
...
class ClientHandler(BaseH):
    ...
    def do_POST(self):
        ...
        if path == "/api/admin/login":
            ...
            if _amod:
                ...
                _entered_hash = _hl.pbkdf2_hmac("sha256", pw.encode(), _salt.encode(), 260000).hex()
                if secrets.compare_digest(_entered_hash, _ADMIN_HASH):
        ...
        if path.startswith("/api/admin/"):
            ...
            _admin_routes = {
                ...
                "/api/admin/clients/export":    lambda: self._admin_clients_export(),
                "/api/admin/clients/restore":   lambda: self._admin_clients_restore(b),
        ...
        routes = {
            ...
            "/api/client/select-plan":   lambda: self._p_client_select_plan(b,store,client),
    ...
    def _p_client_login(self,b):
        ...
        if trial_end and date.fromisoformat(trial_end) < date.today():
            if client.get("status") in ("trial","expired"):
                ...
                plans    = s.get("plans",[])
                bank     = {"iban":s.get("bank_iban",s.get("owner_iban","")),"bank":s.get("bank_name",s.get("owner_bank","")),"name":s.get("owner_account_name",s.get("owner_name",""))}
    ...
    def _p_client_register(self,b):
        ...
        # تعبئة الإعدادات الأولية من بيانات التسجيل
        cs.set_d("settings",{
            "name":        client["hotel_name"],
            "type":        client.get("hotel_type","hotel"),
            "classification": client.get("hotel_type","hotel"),
            "city":        client.get("city",""),
            "rooms":       client.get("rooms",0),
            "region":      client.get("region",""),
            "phone":       client.get("phone",""),
            "email":       client.get("email",""),
            "vat_number":  client.get("vat_number",""),
            "units_count": client.get("units_count",0),
            "mk":          client.get("mk",""),
            "api_key":     client.get("api_key",""),
            "settings_locked": True,  # مقفول فور التسجيل
        })
        # Auto-login
        tok = secrets.token_hex(24)
        cli_ses[tok] = str(client["id"])
...
{{"performance_score": 75, "performance_label": "جيد", "strengths": ["..."], "weaknesses": ["..."], "recommendations": [{{"title":"...","action":"...","impact":"..."}}], "revenue_forecast": {{"current":0,"improved":0,"improvement_pct":0}}, "pricing_advice": "...", "market_comparison": "...", "summary": "..."}}"""
    ...
    # ── HTML Pages ────────────────────────────────────────────
    def _p_client_select_plan(self,b,store,client):
        """العميل يختار خطة — تذكرة تلقائية للأدمن + إرجاع بيانات البنك"""
        if not client:
            self._json({"ok":False,"error":"غير مصرح"}); return
        plan_id   = b.get("plan_id","").strip()
        plan_name = b.get("plan_name","").strip()
        price     = b.get("price",0)
        cname     = client.get("hotel_name","")
        cemail    = client.get("email","")
        cid       = str(client.get("id",""))
        s         = adm.get("admin_settings",{})
        iban      = s.get("bank_iban",  s.get("owner_iban",""))
        bank_name = s.get("bank_name",  s.get("owner_bank",""))
        acc_name  = s.get("owner_account_name", s.get("owner_name",""))
        whatsapp  = s.get("owner_whatsapp","")
        phone     = s.get("owner_phone","")
        ticket = {
            "id":          _id(),
            "title":       f"طلب اشتراك — {plan_name} — {cname}",
            "client_id":   cid,
            "client_name": cname,
            "priority":    "high",
            "status":      "open",
            ...
            "messages": [{
                ...
                "text":  (f"الفندق: {cname}\nالبريد: {cemail}\n"
                          f"الخطة: {plan_name}\nالسعر: {price} ر.س/شهر\n"
                          f"الحالة: في انتظار تأكيد التحويل وإرسال كود التفعيل"),
                "time":  _iso(),
                "auto":  True,
            }],
        }
        adm.append("tickets", ticket)
        store.append("tickets", {"id":ticket["id"],"title":ticket["title"],"status":"open"})
        self._json({
            "ok": True,
            "ticket_id": ticket["id"],
            "bank": {"iban":iban,"bank":bank_name,"name":acc_name,"whatsapp":whatsapp,"phone":phone}
        })
...
إذا كان الأمر يتطلب تنفيذاً، اذكر الإجراءات بدقة."""
    ...
    def _api_payment_plans(self):
        ...
        bank = {
            ...
            "bank":     s.get("bank_name", s.get("owner_bank","")),
            "iban":     s.get("bank_iban",  s.get("owner_iban","")),
            "whatsapp": s.get("owner_whatsapp",""),
            "phone":    s.get("owner_phone",""),
        }
    ...
    def _p_update_upload(self, b):
        """رفع ملف تحديث — يحفظ محلياً ويرفع لـ GitHub تلقائياً"""
        import base64 as _b64, hashlib as _hs, os as _os, urllib.request as _ur
        file_data   = b.get("file_data","")
        file_name   = b.get("file_name","")
        description = b.get("description","تحديث")
        ...
        if safe_name not in allowed:
            self._json({"ok":False,"error":f"الملفات المسموح بها: {', '.join(allowed)}"}); return
        # فحص صياغة Python
        try:
            import ast as _ast
            _ast.parse(raw.decode("utf-8"))
        except SyntaxError as e:
            self._json({"ok":False,"error":f"خطأ في صياغة Python سطر {e.lineno}: {e.msg}"}); return
        # ── GitHub token و repo من الإعدادات أو Railway Variables ──
        s_cfg  = adm.get("admin_settings", {})
        token  = s_cfg.get("github_token","") or os.environ.get("GITHUB_TOKEN","")
        # مستودع الكود (HotelSystem) — مختلف عن مستودع البيانات
        repo   = s_cfg.get("github_repo","")  or os.environ.get("GITHUB_CODE_REPO","") or os.environ.get("GITHUB_REPO","")
        branch = s_cfg.get("github_branch","main") or os.environ.get("GITHUB_BRANCH","main")
        if not token:
            self._json({"ok":False,"error":"أضف GitHub Token في إعدادات المالك أو Railway Variables (GITHUB_TOKEN)"}); return
        if not repo:
            self._json({"ok":False,"error":"أضف مستودع الكود في إعدادات المالك (github_repo) أو Railway Variables (GITHUB_CODE_REPO)"}); return
        ...
        if _os.path.exists(target_path):
            ...
            _sh.copy2(target_path, _os.path.join(backup_dir, f"{safe_name}.backup_{int(_tm.time())}"))
        # ── رفع الملف لـ GitHub ──
        api_url = f"https://api.github.com/repos/{repo}/contents/{safe_name}"
        hdrs = {
            "Authorization": f"token {token}",
            "Accept":        "application/vnd.github.v3+json",
            "Content-Type":  "application/json",
            "User-Agent":    "dheuof-hotel-system",
        }
        # احصل على SHA الحالي للملف
        sha = None
        try:
            req = _ur.Request(api_url, headers=hdrs)
            sha = json.loads(_ur.urlopen(req, timeout=10).read()).get("sha","")
        except Exception: pass
        # ارفع الملف
        payload = {
            "message": f"🔄 {description} [{safe_name}] - {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            "content": file_data,
            "branch":  branch,
        }
        if sha: payload["sha"] = sha
        try:
            req = _ur.Request(api_url, data=json.dumps(payload).encode(), headers=hdrs, method="PUT")
            res = json.loads(_ur.urlopen(req, timeout=30).read())
            commit_url = res.get("commit",{}).get("html_url","")
        except Exception as e:
            err = str(e)
            if "401" in err: msg = "Token غير صحيح أو منتهي — جدّد GitHub Token"
            elif "403" in err: msg = "Token لا يملك صلاحية الكتابة — تأكد من scope: repo"
            elif "404" in err: msg = "المستودع غير موجود أو Token لا يراه"
            elif "422" in err: msg = "الملف لم يتغير أو مشكلة في SHA"
            else: msg = f"خطأ GitHub: {err[:100]}"
        ...
        updates.append({
            ...
            "github_url": commit_url, "repo": repo, "branch": branch,
        })
        adm.set("update_history", updates[-20:])
        analytics.track_action("admin","update_upload",f"{safe_name} {size_kb}KB → GitHub")
        log.info(f"GitHub push: {safe_name} {size_kb}KB → {repo}/{branch}")
        self._json({
            "ok": True, "file": safe_name, "size_kb": size_kb, "md5": chk,
            "github_url": commit_url, "repo": repo, "branch": branch,
            "message": f"✅ تم رفع {safe_name} إلى GitHub — Railway يُحدَّث تلقائياً خلال ~1 دقيقة",
        })

    def _admin_clients_export(self):
        """تصدير كامل لبيانات العملاء واشتراكاتهم — للتنزيل اليدوي"""
        import base64 as _b64
        clients  = adm.get("clients", [])
        keys     = adm.get("license_keys", [])
        payments = adm.get("payments", [])
        settings = adm.get("admin_settings", {})
        export = {
            "export_version": "2.0",
            ...
            "license_keys":   keys,
            "payments":       payments,
            "admin_settings": {k:v for k,v in settings.items()
                               if k not in ("github_token","github_backup_token")},  # لا نصدّر الـ tokens
        }
        data_json = json.dumps(export, ensure_ascii=False, indent=2)
        data_b64  = _b64.b64encode(data_json.encode("utf-8")).decode()
        filename  = f"dheuof_clients_{datetime.now().strftime('%Y%m%d_%H%M')}.json"
        log.info(f"Clients export: {len(clients)} clients")
        self._json({"ok": True, "filename": filename, "data": data_b64,
                    "clients_count": len(clients), "size_kb": round(len(data_json)/1024, 1)})

    def _admin_clients_restore(self, b):
        """استيراد بيانات العملاء من ملف مُصدَّر مسبقاً"""
        import base64 as _b64
        file_data = b.get("file_data","")
        if not file_data:
            self._json({"ok":False,"error":"أرفق ملف البيانات"}); return
        try:
            raw    = _b64.b64decode(file_data)
            export = json.loads(raw.decode("utf-8"))
        except Exception as e:
            self._json({"ok":False,"error":f"ملف غير صالح: {e}"}); return
        if "clients" not in export:
            self._json({"ok":False,"error":"ملف غير مدعوم — يجب أن يحتوي clients"}); return
        # دمج العملاء (لا نحذف الموجودين)
        existing    = adm.get("clients", [])
        exist_ids   = {c.get("id") for c in existing}
        new_clients = [c for c in export.get("clients",[]) if c.get("id") not in exist_ids]
        merged = existing + new_clients
        adm.set("clients", merged)
        # استيراد المفاتيح والمدفوعات
        if export.get("license_keys"):
            exist_keys = {k.get("key") for k in adm.get("license_keys",[])}
            new_keys   = [k for k in export["license_keys"] if k.get("key") not in exist_keys]
            adm.set("license_keys", adm.get("license_keys",[]) + new_keys)
        log.info(f"Clients restore: +{len(new_clients)} new clients, total={len(merged)}")
        self._json({"ok": True, "imported": len(new_clients), "total": len(merged),
                    "message": f"تم استيراد {len(new_clients)} عميل جديد — الإجمالي: {len(merged)}"})
...
document.querySelectorAll('.fade').forEach(el=>obs.observe(el));
    ...
    def _admin_client_unlock_settings(self, amod, b):
        """يفتح إعدادات المنشأة المقفولة من لوحة المالك"""
        cid = b.get("client_id") or b.get("id")
        cs = ClientStore.get(str(cid))
        s = cs.get_d("settings", {})
        s["settings_locked"] = False
        cs.set_d("settings", s)
        log.info(f"Admin unlocked settings for client {cid}")
        self._json({"ok": True, "message": "تم فتح إعدادات المنشأة — يمكن للعميل تعديلها الآن"})
    ...
    def _admin_proxy_post(self, amod, path, b):
        ...
        try:
            handler_map = {
                ...
                "/api/admin/clients/unlock-settings": lambda: self._admin_client_unlock_settings(amod, b),
                # ── المفاتيح ──
...
def main():
    """نقطة الدخول الرئيسية — سيرفر واحد على PORT من Railway"""
    client_port = int(os.environ.get("PORT", 5050))
    admin_port_env = os.environ.get("ADMIN_PORT", "")
    print("="*58, flush=True)
    print("  نظام إدارة الفنادق — Unified SaaS Server", flush=True)
    print("="*58, flush=True)
    print(f"  لوحة العميل  → PORT {client_port}", flush=True)
    print(f"  لوحة المالك  → /admin على نفس المنفذ", flush=True)
    print("="*58, flush=True)
    # Admin server على منفذ منفصل فقط إذا كان ADMIN_PORT مختلفاً
    if admin_port_env and admin_port_env.isdigit():
        admin_port = int(admin_port_env)
        if admin_port != client_port:
            def run_admin_now():
                try:
                    sv = HTTPServer(("0.0.0.0", admin_port), AdminHandler)
                    ...
                    log.warning(f"Admin server failed: {e} — admin accessible via /admin on main port")
            t_admin = threading.Thread(target=run_admin_now, daemon=True)
            t_admin.start()
            log.info(f"Admin server starting on port {admin_port}")
    # Client server في main thread (الرئيسي)
    log.info(f"Client server starting on port {client_port}")
    sv = HTTPServer(("0.0.0.0", client_port), ClientHandler)
    log.info(f"✓ Ready — port:{client_port} — /admin available on same port")
    print(f"SERVER READY ON PORT {client_port}", flush=True)
    try:
        sv.serve_forever()

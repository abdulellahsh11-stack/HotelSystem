    ...
    def _defaults(self):
        return {
            ...
            "admin_settings":{
                ...
                "owner_phone":"", "owner_whatsapp":"", "system_name":"ضيوف",
                "contact_message":"تواصل معنا لتجديد اشتراكك",
                "bank_iban":"", "bank_name":"", "owner_iban":"", "owner_bank":"",
                "owner_account_name":"",
                "trial_days":15,
...
class ClientHandler(BaseH):
    ...
    def do_GET(self):
        ...
        if path in ("/","/dashboard"):
            # إذا عنده توكن صالح → لوحة التحكم، وإلا → Landing Page
        ...
        routes = {
            ...
            "/api/ai/status":     lambda _: self._api_ai_status(store),
    ...
    def do_POST(self):
        ...
        if path == "/api/admin/login":
            ...
            if _amod:
                ...
                if pw == ADMIN_PASS:
                    ...
                    sessions[_tok] = datetime.now() + timedelta(hours=12)  # unified sessions
                    self.send_response(200)
                    self.send_header("Content-Type","application/json; charset=utf-8")
        ...
        routes = {
            ...
            "/api/ai/analyze-data":       lambda: self._p_ai_analyze_data(b,store),
    ...
    def _api_ai_status(self,store):
        """هل AI متاح؟ يتحقق من مفتاح العميل أو مفتاح المالك"""
        client_key = store.get_d("settings",{}).get("claude_key","")
        owner_key  = adm.get("admin_settings",{}).get("claude_key","")
        has_key    = bool(client_key or owner_key)
        source     = "client" if client_key else ("owner" if owner_key else "none")
        self._json({"ok":True,"has_ai":has_key,"source":source})

    def _api_system_info(self):
        """إعدادات النظام العامة المرئية للعميل — من لوحة المالك"""
        s = adm.get("admin_settings", {})
        self._json({
            ...
            "primary_color":   s.get("primary_color", "#185FA5"),
    ...
    def _api_client_notifications(self, store):
        """الإشعارات المرسلة من المالك لهذا العميل"""
        notifs = store.get_d("notifications", [])
        self._json({"ok": True, "notifications": list(reversed(notifs[-20:]))})

    def _p_ai_analyze_data(self, b, store):
        """تحليل بيانات الفندق بالذكاء الاصطناعي"""
        import urllib.request as ur, re as _re
        claude_key = store.get_d("settings",{}).get("claude_key","") or adm.get("admin_settings",{}).get("claude_key","")
        if not claude_key:
            self._json({"ok":False,"error":"لا يوجد مفتاح Claude API — تواصل مع مدير النظام"}); return
        hotel   = b.get("hotel_name","الفندق")
        city    = b.get("city","riyadh")
        rooms   = b.get("rooms",50)
        active  = b.get("active_guests",0)
        occ     = b.get("occupancy",0)
        rev     = b.get("total_revenue",0)
        m_hadr  = b.get("market_hotel_avg",0)
        m_occ   = b.get("market_occ",0)
        prompt  = f"""أنت مستشار فندقي خبير في السوق السعودي.
فندق: {hotel} | مدينة: {city} | غرف: {rooms}
نزلاء حاليون: {active} | إشغال: {occ}%
إجمالي الإيرادات المسجلة: {rev} ر.س
متوسط السوق (ADR): {m_hadr} ر.س | إشغال السوق: {m_occ}%

قدّم تحليلاً احترافياً مختصراً (5-8 نقاط) يشمل:
1. تقييم الأداء الحالي مقارنة بالسوق
2. نقاط القوة والضعف
3. توصيات عملية لتحسين الإيرادات
4. اقتراح سعر مثالي للغرفة بناءً على البيانات

أجب باللغة العربية بتنسيق واضح ومختصر."""
        try:
            payload = json.dumps({
...
<title id="page-title">ضيوف — برنامج محاسبي متكامل للفنادق</title>
...
/* NAV */
nav{position:fixed;top:0;right:0;left:0;z-index:100;background:rgba(10,10,10,0.96);backdrop-filter:blur(12px);border-bottom:1px solid rgba(201,168,76,0.2);padding:0 5%;}
.nav-inner{display:flex;align-items:center;justify-content:space-between;height:64px;}
.nav-logo{font-size:20px;font-weight:900;color:var(--gold);letter-spacing:-0.5px;}
.nav-logo span{color:var(--white);}
.nav-links{display:flex;align-items:center;gap:32px;}
.nav-links a{color:rgba(255,255,255,0.75);text-decoration:none;font-size:14px;font-weight:500;transition:.2s;}
.nav-links a:hover{color:var(--gold);}
.nav-cta{background:var(--gold);color:var(--dark)!important;padding:9px 22px;border-radius:6px;font-weight:700!important;font-size:13px!important;}
...
.hero::before{content:'';position:absolute;inset:0;background:url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><pattern id="g" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse"><circle cx="1" cy="1" r="0.5" fill="%23C9A84C" opacity="0.15"/></pattern></defs><rect width="100" height="100" fill="url(%23g)"/></svg>');opacity:0.4;}
.hero::after{content:'';position:absolute;top:-200px;left:-200px;width:600px;height:600px;background:radial-gradient(circle,rgba(201,168,76,0.12) 0%,transparent 70%);pointer-events:none;}
...
.hero-text h1{font-size:clamp(28px,4vw,52px);font-weight:900;color:var(--white);line-height:1.2;margin-bottom:20px;}
.hero-text h1 em{color:var(--gold);font-style:normal;display:block;}
.hero-text p{color:rgba(255,255,255,0.7);font-size:15px;line-height:1.9;margin-bottom:10px;}
.hero-text p strong{color:var(--gold2);font-weight:700;}
.hero-email-row{display:flex;gap:10px;margin-top:28px;flex-wrap:wrap;}
.hero-input{flex:1;min-width:200px;padding:13px 16px;border:1.5px solid rgba(201,168,76,0.4);border-radius:8px;background:rgba(255,255,255,0.06);color:#fff;font-family:'Tajawal',sans-serif;font-size:14px;outline:none;transition:.2s;}
.hero-input::placeholder{color:rgba(255,255,255,0.35);}
.hero-input:focus{border-color:var(--gold);background:rgba(201,168,76,0.08);}
.hero-btn{padding:13px 28px;background:var(--gold);color:var(--dark);border:none;border-radius:8px;font-family:'Tajawal',sans-serif;font-size:15px;font-weight:800;cursor:pointer;white-space:nowrap;transition:.2s;}
.hero-btn:hover{background:var(--gold2);transform:translateY(-1px);}
.hero-img{position:relative;}
.hero-img img{width:100%;border-radius:16px;box-shadow:0 40px 80px rgba(0,0,0,0.5);display:block;}
.hero-img::before{content:'';position:absolute;inset:-3px;border-radius:18px;background:linear-gradient(135deg,var(--gold),transparent,var(--green));opacity:0.5;z-index:-1;}

/* FEATURES */
.section{padding:90px 5%;}
.section-inner{max-width:1200px;margin:0 auto;}
.section-header{text-align:center;margin-bottom:56px;}
.section-header h2{font-size:clamp(22px,3vw,38px);font-weight:900;color:var(--dark);margin-bottom:12px;}
.section-header p{color:var(--gray);font-size:15px;max-width:560px;margin:0 auto;}
.features-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:28px;}
.feat-card{border:1.5px solid var(--border);border-radius:16px;overflow:hidden;transition:.3s;cursor:pointer;}
.feat-card:hover{border-color:var(--gold);box-shadow:0 12px 40px rgba(201,168,76,0.15);transform:translateY(-4px);}
.feat-img{width:100%;height:200px;object-fit:cover;display:block;background:#f3f4f6;}
.feat-img-placeholder{width:100%;height:200px;display:flex;align-items:center;justify-content:center;font-size:56px;}
.feat-body{padding:24px;}
.feat-body h3{font-size:17px;font-weight:800;color:var(--dark);margin-bottom:10px;}
.feat-body p{font-size:13px;color:var(--gray);line-height:1.8;margin-bottom:18px;}
.feat-btn{display:inline-block;padding:9px 20px;border:2px solid var(--dark);border-radius:6px;font-family:'Tajawal',sans-serif;font-size:13px;font-weight:700;color:var(--dark);background:transparent;cursor:pointer;transition:.2s;}
.feat-btn:hover{background:var(--dark);color:#fff;}

/* LOGIN SECTION */
.login-section{background:var(--dark2);padding:90px 5%;}
.login-inner{max-width:1100px;margin:0 auto;display:grid;grid-template-columns:1fr 420px;gap:70px;align-items:center;}
.login-text h2{font-size:clamp(22px,3vw,40px);font-weight:900;color:var(--white);margin-bottom:16px;line-height:1.3;}
.login-text h2 em{color:var(--gold);font-style:normal;}
.login-text p{color:rgba(255,255,255,0.65);font-size:14px;line-height:1.9;margin-bottom:24px;}
.login-perks{display:flex;flex-direction:column;gap:12px;}
.perk{display:flex;align-items:center;gap:12px;color:rgba(255,255,255,0.8);font-size:14px;}
.perk-icon{width:32px;height:32px;border-radius:8px;background:rgba(201,168,76,0.15);border:1px solid rgba(201,168,76,0.3);display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0;}
.login-card{background:#fff;border-radius:20px;padding:36px;box-shadow:0 30px 80px rgba(0,0,0,0.4);}
.login-tabs{display:flex;border-radius:10px;overflow:hidden;border:1.5px solid var(--border);margin-bottom:24px;}
.ltab{flex:1;padding:10px;text-align:center;cursor:pointer;font-family:'Tajawal',sans-serif;font-size:13px;font-weight:700;color:var(--gray);background:#F8FAFC;transition:.2s;border:none;}
.ltab.on{background:var(--dark2);color:#fff;}
.lpg{display:none;}.lpg.on{display:block;}
.fg{margin-bottom:14px;}
.fg label{font-size:11px;font-weight:700;color:var(--gray);display:block;margin-bottom:5px;text-transform:uppercase;letter-spacing:.5px;}
.fg input,.fg select{width:100%;border:1.5px solid var(--border);border-radius:8px;padding:10px 13px;font-family:'Tajawal',sans-serif;font-size:13px;outline:none;transition:.2s;color:var(--dark);background:#fff;}
.fg input:focus,.fg select:focus{border-color:var(--gold);box-shadow:0 0 0 3px rgba(201,168,76,0.12);}
.lbtn{width:100%;padding:13px;border:none;border-radius:9px;font-family:'Tajawal',sans-serif;font-size:14px;font-weight:800;cursor:pointer;transition:.2s;margin-top:4px;}
.lbtn-login{background:var(--dark2);color:#fff;}
.lbtn-login:hover{background:#1f2937;}
.lbtn-reg{background:linear-gradient(135deg,var(--green),#10B981);color:#fff;}
.lbtn-reg:hover{opacity:.9;}
.lerr{color:#DC2626;font-size:11px;margin-top:8px;text-align:center;display:none;}
.lok{color:#047857;font-size:11px;margin-top:8px;text-align:center;display:none;}
.trial-badge{background:linear-gradient(135deg,#EEF2FF,#E0E7FF);border:1px solid #C7D2FE;border-radius:8px;padding:10px 14px;font-size:12px;color:#3730A3;margin-bottom:16px;text-align:center;font-weight:600;}
.trial-badge span{font-size:16px;}
.divider{text-align:center;font-size:11px;color:var(--gray);margin:14px 0;position:relative;}
...
/* FOOTER */
footer{background:var(--dark);color:rgba(255,255,255,0.5);text-align:center;padding:28px 5%;font-size:13px;}
...
<nav>
  <div class="nav-inner">
    <div class="nav-logo" id="nav-logo">ضيوف <span>للفنادق</span></div>
    <div class="nav-links">
      <a href="#hero">الصفحة الرئيسية</a>
      <a href="#features">عن البرنامج</a>
      <a href="#features">الخدمات</a>
      <a href="#login">تواصل بنا</a>
      <a href="#login" class="nav-cta" onclick="switchLoginTab('register')">ابدأ مجاناً</a>
    </div>
  </div>
</nav>
...
<section class="hero" id="hero">
  <div class="hero-inner">
    <div class="hero-text fade-up">
      <h1>استمتع بإدارة مالية متكاملة لفندقك مع برنامج<em id="hero-sys-name">ضيوف المتطور!</em></h1>
      <p><strong>حل شامل:</strong> نقدم برنامجنا مجموعة متكاملة من الأدوات لإدارة الحسابات، الفواتير، والمخزون بشكل فعال.</p>
      <p><strong>تصميم مريح:</strong> تم تصميم واجهتنا لتكون سهلة الاستخدام، مما يوفر لك تجربة مريحة وسلسة.</p>
      <p><strong>تحليلات مالية دقيقة:</strong> احصل على تقارير مالية شاملة تعزز من قرارات الاستراتيجية.</p>
      <p><strong>الموازنة:</strong> البرنامج متكامل بأعلى معايير الأداء الشامل للاستمرار بدون أي التزامات ومديونيات مفاجأة.</p>
      <div class="hero-email-row">
        <input class="hero-input" id="hero-email" type="email" placeholder="أدخل بريدك الإلكتروني"/>
        <button class="hero-btn" onclick="heroRegister()">أريد التجربة!</button>
      </div>
    </div>
    <div class="hero-img fade-up" style="animation-delay:.2s">
      <img src="https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&q=80" alt="فندق فاخر" loading="lazy" onerror="this.style.display='none'"/>
    </div>
  </div>
</section>
...
<section class="section" id="features">
  <div class="section-inner">
    <div class="section-header fade-up">
      <h2>ميزات فريدة لبرنامج المحاسبة</h2>
      <p>استفد من مجموعة من الميزات المصممة خصيصاً لتلبية احتياجات قطاع الضيافة.</p>
    </div>
    <div class="features-grid">
      <div class="feat-card fade-up">
        <div class="feat-img-placeholder" style="background:#0a1628">🔒</div>
        <div class="feat-body">
          <h3>أمان وحماية البيانات</h3>
          <p>نحن نضمن حماية بياناتك المالية من خلال تقنيات أمان متطورة، مما يمنحك راحة البال في إدارة مالك والحساسة.</p>
          <button class="feat-btn" onclick="scrollToLogin()">تأمين بياناتك</button>
        </div>
      </div>
      <div class="feat-card fade-up" style="transition-delay:.1s">
        <div class="feat-img-placeholder" style="background:#0f1f0f">💳</div>
        <div class="feat-body">
          <h3>تكامل مع الأنظمة الأخرى</h3>
          <p>يمكنك دمج البرنامج مع أنظمة أخرى مثل أنظمة الدفع الإلكتروني، مما يسهل عليك عمليات الفوترة المالية بشكل متكامل.</p>
          <button class="feat-btn" onclick="scrollToLogin()">تعرف على التكامل</button>
        </div>
      </div>
      <div class="feat-card fade-up" style="transition-delay:.2s">
        <div class="feat-img-placeholder" style="background:#1a1a2e">📱</div>
        <div class="feat-body">
          <h3>واجهة مستخدم سهلة الاستخدام</h3>
          <p>تم تصميم البرنامج بواجهة مستخدم بسيطة وسهلة الصنف، مما يسهل على جميع الموظفين استخدامه بسرعة وكفاءة.</p>
          <button class="feat-btn" onclick="scrollToLogin()">استمتع بالتجربة</button>
        </div>
      </div>
    </div>
  </div>
...
<section class="login-section" id="login">
  <div class="login-inner">
    <div class="login-text fade-up">
      <h2>ابدأ رحلتك مع<em id="login-sys-name"> ضيوف</em> اليوم</h2>
      <p>انضم إلى مئات الفنادق والشقق المخدومة في السعودية التي تثق بنظامنا لإدارة أعمالها بكفاءة وأمان.</p>
      <div class="login-perks">
        <div class="perk"><div class="perk-icon">⭐</div><span>تجربة مجانية <span id="perk-trial">15</span> يوم بدون بطاقة ائتمان</span></div>
        <div class="perk"><div class="perk-icon">🔒</div><span>بياناتك محفوظة ومشفرة بالكامل</span></div>
        <div class="perk"><div class="perk-icon">📊</div><span>تقارير مالية يومية وشهرية فورية</span></div>
        <div class="perk"><div class="perk-icon">💬</div><span>دعم فني عبر واتساب <span id="perk-wa"></span></span></div>
        <div class="perk"><div class="perk-icon">🏨</div><span>يعمل مع الفنادق والشقق المخدومة والاستراحات</span></div>
      </div>
    </div>
    <div class="login-card fade-up" style="transition-delay:.15s">
      <div style="text-align:center;margin-bottom:20px">
        <div style="font-size:32px" id="card-logo">🏨</div>
        <div style="font-size:17px;font-weight:900;color:var(--dark);margin-top:6px" id="card-sys-name">ضيوف للفنادق</div>
      </div>
      <div class="login-tabs">
        <button class="ltab on" onclick="switchLoginTab('login')">تسجيل الدخول</button>
        <button class="ltab" onclick="switchLoginTab('register')">تجربة مجانية</button>
      </div>

      <!-- LOGIN TAB -->
      <div class="lpg on" id="lpg-login">
        <div class="fg">
          <label>البريد الإلكتروني</label>
          <input type="email" id="l-email" placeholder="hotel@example.com" onkeydown="if(event.key==='Enter')doLogin()"/>
        </div>
        <button class="lbtn lbtn-login" onclick="doLogin()">دخول إلى لوحة التحكم ←</button>
        <div class="lerr" id="l-err"></div>
        <div class="divider">أو</div>
        <button class="lbtn" style="background:transparent;border:1.5px solid var(--border);color:var(--gray)" onclick="switchLoginTab('register')">ابدأ تجربتك المجانية</button>
      </div>

      <!-- REGISTER TAB -->
      <div class="lpg" id="lpg-register">
        <div class="trial-badge"><span>⭐</span> تجربة مجانية <span id="trial-days-badge">15</span> يوم — بدون بطاقة ائتمان</div>
        ...
        <div class="fg"><label>البريد الإلكتروني *</label><input type="email" id="r-email" placeholder="you@hotel.sa"/></div>
        ...
        <button class="lbtn lbtn-reg" onclick="doRegister()">ابدأ التجربة المجانية ←</button>
        <div class="lerr" id="r-err"></div>
        <div class="lok" id="r-ok"></div>
      </div>
    </div>
  </div>
</section>
...
<footer>
  ...
  <div id="footer-copy">© 2026 Dheuof — جميع الحقوق محفوظة</div>
</footer>

<script>
// ── Load system info from admin ──
async function loadSysInfo(){
  try{
    const d=await fetch('/api/system-info').then(r=>r.json());
    const sn=d.system_name||'ضيوف';
    const em=d.logo_emoji||'🏨';
    const td=d.trial_days||15;
    const wa=d.owner_whatsapp||'';
    // Apply everywhere
    ['hero-sys-name','login-sys-name','card-sys-name','nav-logo'].forEach(id=>{
      const el=document.getElementById(id);
      if(el){
        if(id==='nav-logo') el.innerHTML=sn+' <span>للفنادق</span>';
        else if(id==='hero-sys-name') el.textContent=sn+' المتطور!';
        else el.textContent=sn;
      }
    });
    const logoEl=document.getElementById('card-logo');if(logoEl)logoEl.textContent=em;
    document.title=sn+' — برنامج محاسبي متكامل للفنادق';
    ['trial-days-badge','perk-trial'].forEach(id=>{const el=document.getElementById(id);if(el)el.textContent=td;});
    const waEl=document.getElementById('perk-wa');if(waEl&&wa)waEl.textContent='('+wa+')';
    if(d.primary_color){
      document.documentElement.style.setProperty('--gold',d.primary_color);
    }
  }catch(e){}
}
loadSysInfo();
...
function switchLoginTab(t){
  document.querySelectorAll('.ltab').forEach((el,i)=>el.classList.toggle('on',i===(t==='login'?0:1)));
  document.querySelectorAll('.lpg').forEach((el,i)=>el.classList.toggle('on',i===(t==='login'?0:1)));
  if(t==='register') scrollToLogin();
}
...
function heroRegister(){
  ...
  if(email){document.getElementById('r-email').value=email;}
...
async function doLogin(){
  ...
  if(!email){showErr('l-err','أدخل البريد الإلكتروني');return;}
  const btn=document.querySelector('#lpg-login .lbtn-login');
  if(btn){btn.textContent='جارٍ الدخول...';btn.disabled=true;}
  try{
    const r=await fetch('/api/client/login',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({email})});
    const d=await r.json();
    if(d.ok){location.href='/dashboard';}
    else if(d.error==='expired'){showExpiredModal(d);}
    else{showErr('l-err',d.error||'البريد غير مسجل أو غير صحيح');}
...
async function doRegister(){
  ...
  if(!name||!email){showErr('r-err','الاسم والبريد الإلكتروني مطلوبان');return;}
  const btn=document.querySelector('#lpg-register .lbtn-reg');
  if(btn){btn.textContent='جارٍ التسجيل...';btn.disabled=true;}
  try{
    const r=await fetch('/api/client/register',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({
...
// ── Expired modal ──
function showExpiredModal(d){
  const wa=d.whatsapp||'';
  const plans=(d.plans||[]).filter(p=>p.price>0);
  const bank=d.bank||{};
  let sel=plans[0]||{id:'pro',name:'احترافية',price:299};
  const overlay=document.createElement('div');
  overlay.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;';
  overlay.innerHTML=`<div style="background:#fff;border-radius:20px;padding:32px;max-width:460px;width:100%;max-height:90vh;overflow-y:auto;font-family:Tajawal,sans-serif;direction:rtl">
    <div style="text-align:center;margin-bottom:20px">
      <div style="font-size:42px">⏰</div>
      <div style="font-size:20px;font-weight:900;color:#0a0a0a;margin:8px 0">انتهت فترة التجربة</div>
      <div style="font-size:13px;color:#6B7280">${d.message||'جدد اشتراكك للاستمرار'}</div>
    </div>
    ${plans.map(p=>`<div style="border:2px solid ${p.color||'#185FA5'};border-radius:12px;padding:14px;margin-bottom:10px;cursor:pointer;text-align:center" onclick="sel={id:'${p.id}',name:'${p.name}',price:${p.price}}">
      <div style="font-weight:800;color:${p.color||'#185FA5'}">${p.name}</div>
      <div style="font-size:26px;font-weight:900;color:${p.color||'#185FA5'}">${p.price} <span style="font-size:13px">ر.س/شهر</span></div>
    </div>`).join('')}
    ${bank.iban?`<div style="background:#F0FDF4;border:1px solid #86EFAC;border-radius:10px;padding:14px;margin:12px 0;font-size:12px">
      <div style="font-weight:700;color:#166534;margin-bottom:6px">🏦 التحويل البنكي</div>
      ${bank.name?`<div>الاسم: <b>${bank.name}</b></div>`:''}
      ${bank.bank?`<div>البنك: <b>${bank.bank}</b></div>`:''}
      <div style="font-family:monospace;font-size:13px;font-weight:700;color:#166534;margin-top:4px">${bank.iban}</div>
    </div>`:''}
    ${wa?`<a href="https://wa.me/${wa}" target="_blank" style="display:block;text-align:center;padding:13px;background:#25D366;color:#fff;border-radius:10px;text-decoration:none;font-weight:800;margin-top:8px">💬 تواصل واتساب للتجديد</a>`:''}
    <button onclick="this.closest('div[style]').remove()" style="width:100%;padding:10px;margin-top:10px;background:transparent;border:1.5px solid #E5E7EB;border-radius:8px;color:#6B7280;font-family:Tajawal,sans-serif;font-size:13px;cursor:pointer">إغلاق</button>
  </div>`;
